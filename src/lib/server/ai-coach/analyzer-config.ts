...
export class ConfigAnalyzer extends AnalyzerBase {
  ...
  constructor(
    ...
    sharedMap?: Map<import("./types").SessionRequest, Session>,
  ) {
  ...
  getConfigHealth(f?: DateFilter): ConfigHealthData {
    ...
    const cutoffDate =
      f?.fromDate ?? toDateStr(Date.now() - 90 * 24 * 60 * 60 * 1000);
    const wsHealths = this.getTargetWorkspaceIds(f)
      .map((wsId) =>
        this.buildWorkspaceHealth(wsId, wsActivity.get(wsId), cutoffDate, f),
      )
    ...
    const agenticReadiness = this.computeAgenticReadiness(
      deduped,
      personalSkillFiles.length,
    );
    const overallScore = this.computeOverallScore(
      deduped,
      contextProvisionByHarness,
    );
    const contextAntiPatterns = this.deriveContextAntiPatterns(
      deduped,
      contextProvisionByHarness,
    );
  ...
  private getWorkspaceAnalysisContext(
    ...
    const harness =
      activity?.harness || (isClaudeWorkspace ? "Claude Code" : "Local Agent");
  ...
  private buildResolvedWorkspaceHealth(
    ...
    const hookCoverage = isClaudeWorkspace
      ? analyzeHookCoverage(rootPath)
      : null;
    ...
    return {
      ...
      harness,
      configFiles,
      hasInstructions: configFiles.some(
        (cf) => cf.kind === "instruction" || cf.kind === "claude-md",
      ),
      ...
      progressiveDisclosureScore:
        computeProgressiveDisclosureScore(configFiles),
  ...
  private buildUnresolvedWorkspaceHealth(
    ...
    return {
      ...
      hookCoverage: null,
      suggestions: [
        "Unable to resolve workspace root path. Config file analysis unavailable.",
      ],
      sessionCount: activity?.sessionCount ?? 0,
      requestCount: activity?.requestCount ?? 0,
      lastActivity: activity?.lastTimestamp ?? null,
  ...
  private getStaleStatus(
    ...
    if (lastActivityTs && configFiles.length > 0) {
      const newestFileMod = Math.max(
        ...configFiles.map((cf) => cf.lastModified ?? 0),
      );
      if (newestFileMod > 0 && lastActivityTs > newestFileMod) {
        const daysSinceUpdate = Math.floor(
          (Date.now() - newestFileMod) / (1000 * 60 * 60 * 24),
        );
    ...
    for (const workspace of wsHealths) {
      ...
      if (existing !== undefined) {
        ...
        if (
          ...
          (!prev.lastActivity || workspace.lastActivity > prev.lastActivity)
        ) {
  ...
  private computeWorkspaceActivity(_f?: DateFilter): Map<
    string,
    {
      sessionCount: number;
      requestCount: number;
  ...
  > {
    ...
    for (const s of this.sessions) {
      ...
      if (
        ts != null &&
        (entry.lastTimestamp == null || ts > entry.lastTimestamp)
      ) {
  ...
  private deriveContextAntiPatterns(
    ...
    const activeNoContext = wsHealths.filter(
      (w) => w.requestCount >= 100 && !w.hasInstructions,
    );
    if (activeNoContext.length > 0) {
      ...
      patterns.push({
        ...
        suggestion:
          "Add .github/copilot-instructions.md (for VS Code) or CLAUDE.md (for Claude Code) to your most active workspaces. This is the single highest-leverage action for better AI outputs.",
        examples: activeNoContext
          .slice(0, 5)
          .map(
            (w) =>
              `${w.workspaceName} (${w.requestCount} requests, ${w.harness})`,
          ),
        details,
    ...
    const staleWs = wsHealths.filter(
      (w) => w.staleContext && w.staleDays != null,
    );
    if (staleWs.length > 0) {
      ...
      patterns.push({
        ...
        suggestion:
          "Review and update your instruction files periodically. As your codebase evolves, your AI context should evolve too.",
        examples: staleWs
          .slice(0, 5)
          .map(
            (w) => `${w.workspaceName}: context files ${w.staleDays} days old`,
          ),
    ...
    for (const [harness, score] of Object.entries(contextProvision)) {
      if (score.score < 20 && score.totalRequests >= 50) {
        const fileRefPct = Math.round(
          (score.withFileRefs / score.totalRequests) * 100,
        );
        const instrPct = Math.round(
          (score.withCustomInstructions / score.totalRequests) * 100,
        );
  ...
  private computeContextProvisionByHarness(
    f?: DateFilter,
  ): Record<string, ContextProvisionScore> {
    ...
    for (const [harness, hReqs] of byHarness) {
      ...
      const avgRequestsPerSession =
        totalSessions > 0
          ? Math.round((hReqs.length / totalSessions) * 10) / 10
          : 0;
      const avgPromptLength =
        hReqs.length > 0
          ? Math.round(
              hReqs.reduce((s, r) => s + r.messageLength, 0) / hReqs.length,
            )
          : 0;
      const avgResponseLength =
        hReqs.length > 0
          ? Math.round(
              hReqs.reduce((s, r) => s + r.responseLength, 0) / hReqs.length,
            )
          : 0;
      const canceledCount = hReqs.filter((r) => r.isCanceled).length;
      const cancelRate =
        hReqs.length > 0 ? Math.round((canceledCount / hReqs.length) * 100) : 0;
      const agentModeCount = hReqs.filter(
        (r) =>
          r.agentMode !== "" && r.agentMode !== "chat" && r.agentMode !== "ask",
      ).length;
      const agentModeRate =
        hReqs.length > 0
          ? Math.round((agentModeCount / hReqs.length) * 100)
          : 0;

      // Mode distribution
      const modeCounts = new Map<string, number>();
      for (const r of hReqs) {
        const m = r.agentMode || "unknown";
      ...
      for (const r of hReqs)
        for (const rf of r.referencedFiles) {
          fileCounts.set(rf, (fileCounts.get(rf) ?? 0) + 1);
        }
      const topReferencedFiles = [...fileCounts.entries()]
        .sort((a, b) => b[1] - a[1])
        .slice(0, 5)
  ...
  private computeAgenticReadiness(
    wsHealths: WorkspaceConfigHealth[],
  ...
  getContextReviewPayload(
    ...
    for (const wsId of wsIds) {
      ...
      const readmeSnippet = readSnippet(
        rootPath,
        ["README.md", "readme.md", "Readme.md"],
        maxFileChars,
      );
      const packageSnippet = readSnippet(
        rootPath,
        [
          "package.json",
          "Cargo.toml",
      ...
      const contextFiles = configFiles.map((cf) => {
        ...
        let content = "";
        try {
          content = fs.readFileSync(fullPath, "utf-8").slice(0, maxFileChars);
        } catch {
          /* skip */
        }
  ...
  private computeOverallScore(
    ...
    if (wsHealths.length === 0 && Object.keys(contextProvision).length === 0)
      return 0;

    const configScore =
      wsHealths.length > 0
        ? wsHealths.reduce((sum, w) => sum + w.progressiveDisclosureScore, 0) /
          wsHealths.length
        : 0;

    const provisions = Object.values(contextProvision);
    const provisionScore =
      provisions.length > 0
    ...
    const qualityScore =
      wsHealths.length > 0
        ? wsHealths.reduce((sum, w) => sum + w.instructionQualityScore, 0) /
          wsHealths.length
        : 0;

    return Math.round(
      configScore * 0.4 + provisionScore * 0.35 + qualityScore * 0.25,
    );
  }
}
