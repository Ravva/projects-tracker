...
export function findVsCodeDirs(): string[] {
  ...
  for (const edition of editionFolders) {
    ...
    if (vsPath && fs.existsSync(vsPath) && !dirs.includes(vsPath))
      dirs.push(vsPath);
  }
...
function resolveWorkspaceName(
  ...
  if (isCLI)
    return fs.existsSync(wsYamlPath) ? parseCLIWorkspaceName(wsYamlPath) : wsId;
...
function detectCustomInstructionsBytes(
...
function resolveCustomInstructionsBytes(
  entryPath: string,
  isCLI: boolean,
): number | undefined {
  const cached = INSTRUCTIONS_BYTES_CACHE.get(entryPath);
  if (cached !== undefined || INSTRUCTIONS_BYTES_CACHE.has(entryPath))
    return cached;
  ...
  try {
    if (isCLI) {
      ...
      if (fs.existsSync(wsJson) || prefetchCache.has(wsJson))
        folder = parseWorkspaceFolderPath(wsJson);
    }
  } catch {
    /* ignore */
  }
  ...
  try {
    return fs
      ...
      .filter(
        (cf) =>
          cf.isFile() &&
          (cf.name.endsWith(".json") || cf.name.endsWith(".jsonl")),
      )
...
function listEditStateFiles(esDir: string): string[] {
  try {
    return fs
      .readdirSync(esDir, { withFileTypes: true })
...
function processEditOperation(
  op: EditStateOperation,
  editLocIndex: ParseContext["editLocIndex"],
): void {
...
function processEditOperations(
  operations: EditStateOperation[] | undefined,
  editLocIndex: ParseContext["editLocIndex"],
): void {
...
function initializeWorkspaceEntry(
  ...
  wsName: string;
  isCLI: boolean;
  customInstructionsBytes: number | undefined;
} {
  ...
  const customInstructionsBytes = resolveCustomInstructionsBytes(
    entryPath,
    isCLI,
  );
...
export function processWorkspaceEntry(
  ...
  const { entryPath, wsName, isCLI, customInstructionsBytes } =
    initializeWorkspaceEntry(logsDir, wsId, harness, workspaces);

  if (isCLI) {
    const eventsFile = path.join(entryPath, "events.jsonl");
    const cliSession = parseCLIEventsFile(
      eventsFile,
      wsId,
      wsName,
...
export async function processWorkspaceEntryAsync(
  ...
  const editStateFiles = listEditStateFiles(
    path.join(entryPath, "chatEditingSessions"),
  );
...
function extractAgentInfo(agent: RawRequest["agent"]): {
  agentName: string;
  agentMode: string;
} {
...
function normalizeSessionMode(modeId: string | undefined): string {
  ...
  let decoded: string;
  try {
    decoded = decodeURIComponent(modeId);
  } catch {
    decoded = modeId;
  }
...
function extractCustomInstructions(
  ...
  for (const cr of contentRefs || []) {
    ...
    if (
      lower.includes(".instructions.md") ||
      lower.includes("copilot-instructions") ||
      lower.includes(".prompt.md") ||
...
function extractSkillsFromPromptFiles(
  vdVars: RawVariable[],
  skills: Set<string>,
): void {
  for (const v of vdVars) {
...
function extractSkillsFromToolCalls(
  result: RawRequest["result"],
  skills: Set<string>,
): void {
  const resultMeta =
    (typeof result === "object" && result ? result.metadata : null) || {};
  ...
  for (const key of ["toolCallResults", "toolCallRounds"]) {
    ...
    for (const tcr of arr) {
      ...
      const tcData = parseToolCalls(tcrObj.toolCalls, (error) => {
        debugCore(
          "parser-vscode",
          "Failed to parse toolCalls JSON string",
          error,
        );
      });
...
function parseTodoListFromToolCall(
  tool: TodoToolCall,
): import("./types").TodoItem[] | null {
  ...
  try {
    ...
    return items.map(
      (it: { id?: number; title?: string; status?: string }) => ({
        id: it.id ?? 0,
        title: String(it.title ?? ""),
        status:
          it.status === "in-progress" || it.status === "completed"
            ? it.status
            : "not-started",
      }),
    );
...
function extractToolConfirmations(
  resp: unknown[] | undefined,
): ToolConfirmation[] {
  ...
  for (const part of resp) {
    ...
    confirmations.push({
      ...
      commandLine: isTerminal
        ? String(
            tsd?.confirmation?.commandLine || tsd?.commandLine?.original || "",
          )
        : undefined,
    });
  }
...
function extractRequestVariables(
  req: RawRequest,
  resp: RawRequest["response"],
  result: RawRequest["result"],
): {
...
function extractResultMetadata(
  ...
  const meta = (
    typeof resultMeta === "object" && resultMeta ? resultMeta : {}
  ) as Record<string, unknown>;
  return {
    resultObj,
    meta,
    resultIsFinalized:
      !!resultObj && Object.keys(resultObj).length > 0 && !!resultMeta,
  };
}

function extractTokenInfo(
  req: RawRequest,
  parsedResult: ParsedResultMetadata,
): TokenInfo {
  ...
  const topLevelCompletionTokens =
    resultIsFinalized &&
...
function extractCompaction(
  meta: ParsedResultMetadata["meta"],
): import("./types").CompactionEvent | null {
  ...
  return {
    ...
    numRoundsSinceLastSummarization:
      typeof s.numRoundsSinceLastSummarization === "number"
        ? s.numRoundsSinceLastSummarization
        : 0,
    contextLengthBefore:
      typeof s.contextLengthBefore === "number" ? s.contextLengthBefore : 0,
...
function parseRawRequest(req: RawRequest): SessionRequest {
  ...
  const { firstProgress, totalElapsed, agentName, agentMode, slashCommand } =
    extractRequestMetadata(req, result);
  const {
    variableKinds,
    customInstructions,
    skillsUsed,
  ...
  const { promptTokens, completionTokens } = extractTokenInfo(req, {
    resultObj,
    meta,
    resultIsFinalized,
  });
...
export function parseSessionFile(
  ...
  try {
    if (sessionFile.endsWith(".jsonl")) {
      ...
      data = JSON.parse(
        stripImageData(readFile(sessionFile)),
      ) as SessionFileData;
    }
  } catch (e) {
    debugCore(
      "parser-vscode",
      `Cannot read/parse session file ${sessionFile}`,
      e,
    );
    return null;
  }
  ...
  const sessionEffortDefault = canonicalizeReasoningEffort(
    data.inputState?.selectedModel?.metadata?.configurationSchema?.properties
  ...
  return createSession({
    sessionId:
      data.sessionId || path.basename(sessionFile, path.extname(sessionFile)),
    workspaceId: wsId,
    workspaceName: wsName,
