...
const INTERACTIVE_ENTRYPOINTS: ReadonlySet<string> = new Set([
  "cli",
  "claude-desktop",
]);

function classifyLauncher(
  entrypoint: string | undefined,
): "interactive" | "programmatic" {
  return entrypoint && INTERACTIVE_ENTRYPOINTS.has(entrypoint)
    ? "interactive"
    : "programmatic";
}

function harnessForLauncher(
...
function isClaudeMessage(value: unknown): value is ClaudeMessage {
  ...
  if (value.content !== undefined) {
    ...
    if (
      ...
      (!Array.isArray(content) || !content.every(isClaudeContentBlock))
    ) {
      return false;
    }
  }
...
function isClaudeLine(value: unknown): value is ClaudeLine {
  ...
  if (
    ...
    typeof value.parentUuid !== "number"
  )
    return false;
  ...
  if (value.entrypoint !== undefined && typeof value.entrypoint !== "string")
    return false;
  if (value.isSidechain !== undefined && typeof value.isSidechain !== "boolean")
    return false;
  if (value.agentId !== undefined && typeof value.agentId !== "string")
    return false;
  if (value.message !== undefined && !isClaudeMessage(value.message))
    return false;
  return true;
}
...
function parseClaudeLine(rawLine: string): ClaudeLine | null {
  try {
    ...
    if (typeof record.parentUuid === "number")
      record.parentUuid = String(record.parentUuid);
...
function countClaudeImages(line: ClaudeLine): number {
  return toContentArray(line.message?.content).filter(
...
function applyClaudeToolBlock(
  ...
  data: Pick<
    ClaudeAssistantData,
    "toolsUsed" | "editedFiles" | "referencedFiles" | "skillsUsed"
  >,
): void {
  ...
  if (block.name === "Skill") {
    ...
    if (skillName && !skillName.includes("ai_toolkit"))
      data.skillsUsed.push(skillName);
    return;
  }
...
function collectClaudeAssistantData(
  lines: ClaudeLine[],
  startIndex: number,
  lastTs: number | null,
): ClaudeAssistantData {
  ...
  while (i < lines.length) {
    ...
    if (next.type === "assistant") {
      ...
      data.totalInputTokens +=
        getNumberField(usage, "input_tokens") + cacheRead + cacheWrite;
        ...
        if (
          ...
          const code =
            typeof block.input.content === "string"
              ? block.input.content
              : typeof block.input.new_str === "string"
                ? block.input.new_str
                : null;
          if (code && filePath) {
  ...
  while (offset < remaining.length) {
    ...
    try {
      dirEntries = fs
        ...
        .map((e) => ({
          name: e.name,
          encoded: encodeComponentForMatch(e.name),
        }))
...
function parseClaudeProjectSessions(
  ...
  for (const entry of entries) {
    ...
    const session = parseClaudeSessionFile(
  ...
  for (const entry of entries) {
    ...
    for (const subEntry of subagentEntries) {
      ...
      if (parent) {
        ...
        if (
          subSession.lastMessageDate &&
          (!parent.lastMessageDate ||
            subSession.lastMessageDate > parent.lastMessageDate)
        ) {
        ...
        if (!orphanEntrypoint && subSession.entrypoint)
          orphanEntrypoint = subSession.entrypoint;
      }
    }
    ...
    if (orphanRequests.length > 0) {
      warnCore(
        "parser-claude",
        `subagent dir without parent session: ${entry.name}`,
        {
...
function buildClaudeRequest(
  ...
  return createRequest({
    ...
    totalElapsed:
      userTs && assistantData.lastTs ? assistantData.lastTs - userTs : null,
    ...
    cacheReadTokens:
      assistantData.totalCacheReadTokens > 0
        ? assistantData.totalCacheReadTokens
        : null,
    cacheWriteTokens:
      assistantData.totalCacheWriteTokens > 0
        ? assistantData.totalCacheWriteTokens
        : null,
...
function parseClaudeSessionFile(
  ...
  while (i < lines.length) {
    ...
    requests.push(
      buildClaudeRequest(line, assistantData, userTs, requests.length),
    );
