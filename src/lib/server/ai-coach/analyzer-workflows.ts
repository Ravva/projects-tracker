...
function isNoise(text: string): boolean {
  ...
  if (
    /^@?\w*\s*(continue|try again|yes|no|cancel|abort|stop|retry)\s*[:?!.]*\s*("?continue to iterate\??"?)?$/i.test(
      t,
    )
  )
    return true;
  if (lower === "y" || lower === "n" || lower === "yes" || lower === "no")
    return true;
  if (
    /^(@\w+\s+)?continue:?\s*("?continue to iterate\??"?|to iterate|$)/i.test(t)
  )
    return true;
...
function draftSkill(cluster: {
  label: string;
  records: PromptRecord[];
}): string {
  const topExamples = cluster.records
    .slice(0, 3)
  ...
  getWorkflowOptimization(f?: DateFilter): WorkflowOptimizationData {
    ...
    const clusters: WorkflowCluster[] = qualified.map((c, idx) => {
      ...
      for (const m of members) {
        sessionCounts.set(
          m.sessionId,
          (sessionCounts.get(m.sessionId) || 0) + 1,
        );
      }
      const sessionsWithMultiple = [...sessionCounts.values()].filter(
        (v) => v > 1,
      );
      const avgCorrectionTurns =
        sessionsWithMultiple.length > 0
          ? sessionsWithMultiple.reduce((a, b) => a + b, 0) /
            sessionsWithMultiple.length
          : 0;
      ...
      return {
        ...
        cancelRate:
          members.length > 0
            ? Math.round((cancelCount / members.length) * 100)
            : 0,
        firstSeen:
