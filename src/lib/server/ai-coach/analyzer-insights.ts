...
const PLANNING_RE =
  /\b(plan|architect|design|outline|approach|strategy|scope|breakdown|roadmap|RFC|spec|proposal)\b/i;
const DEBUG_RE =
  /\b(fix|bug|error|exception|crash|debug|stacktrace|trace|issue|broken|fail|wrong|not working|undefined is not|cannot read|segfault|panic)\b/i;
const REVIEW_RE =
  /\b(review|explain|understand|what does|how does|walk me through|read|audit|analyze|inspect|clarify|describe)\b/i;
const EXPLORE_RE =
  /\b(how to|what is|can I|learn|explore|example|tutorial|demo|try|experiment|compare|research|playground)\b/i;
...
function classifyIntent(session: Session): SessionIntent {
  ...
  for (const r of session.requests) {
    ...
    if (r.aiCode.length > 0 || r.editedFiles.length > 0)
...
const SPEC_KEYWORD_RE =
  /\b(spec|requirements?|acceptance criteria|design doc|PRD|RFC|plan file|constraint|must|should|ensure)\b/i;
...
function isSpecDriven(session: Session): boolean {
  ...
  // Uses plan mode
  if (first.agentMode.includes("plan") || first.slashCommand === "plan")
    return true;
  return false;
}
...
const CONSTRAINT_RE =
  /\b(must|should|shall|only|no more than|at most|at least|limit|constraint|require|restrict)\b/i;
const SUCCESS_CRITERIA_RE =
  /\b(expect|success|criteria|acceptance|verify|assert|should return|should output|output should|result should)\b/i;
const VERIFICATION_RE =
  /\b(test|verify|validate|check|confirm|ensure|assert|prove)\b/i;
...
function gradePrompt(
  ...
  if (contextProvision === 0)
    issues.push("No context provided (no file refs, no code)");
...
export class InsightsAnalyzer extends AnalyzerBase {
  ...
  private getLearningVelocity(f?: DateFilter): LearningVelocityData {
    ...
    for (const r of reqs) {
      ...
      for (const block of r.aiCode)
        if (block.language) s.add(block.language.toLowerCase());
  ...
  private getIntentClassification(f?: DateFilter): IntentClassificationData {
    ...
    const dist: Record<SessionIntent, number> = {
      Planning: 0,
      Implementation: 0,
      Debugging: 0,
      Review: 0,
    ...
    const reqsByIntent: Record<SessionIntent, number[]> = {
      Planning: [],
      Implementation: [],
      Debugging: [],
      Review: [],
    ...
    const series: Record<SessionIntent, number[]> = {
      Planning: [],
      Implementation: [],
      Debugging: [],
      Review: [],
    ...
    const avgByIntent: Record<SessionIntent, number> = {} as Record<
      SessionIntent,
      number
    >;
    for (const intent of SESSION_INTENTS) {
  ...
  private getSpecDriven(f?: DateFilter): SpecDrivenData {
    const sessions = this.filteredSessions(f).filter(
      (s) => s.requestCount >= 3,
    );
    const specDriven = sessions.filter(isSpecDriven);
    const weekBuckets = new Map<
      string,
      { spec: number; unstructured: number }
    >();
  ...
  private getProductionReview(f?: DateFilter): ProductionReviewData {
    ...
    const weekBuckets = new Map<
      string,
      { produced: number; reviewed: number }
    >();
    ...
    for (const s of sessions) {
      ...
      if (week && !weekBuckets.has(week))
        weekBuckets.set(week, { produced: 0, reviewed: 0 });
    ...
    const avgGap =
      reviewGaps.length > 0
        ? Math.round(
            reviewGaps.reduce((a, b) => a + b, 0) / reviewGaps.length / 1000,
          )
        : 0;
  ...
  private getSustainablePace(f?: DateFilter): SustainablePaceData {
    ...
    const alerts = this.getSustainablePaceAlerts(
      reqs,
      currentStreak,
      lateNightReqs,
      weekendReqs,
    ...
    return {
      ...
      burnoutRisk: this.getBurnoutRisk(
        alerts,
        currentStreak,
        lateNightTrending,
      ),
  ...
  private getPromptMaturity(f?: DateFilter): PromptMaturityData {
    ...
    const step = Math.max(
      1,
      Math.floor(reqs.length / PROMPT_MATURITY_SAMPLE_SIZE),
    );
    const sampled = reqs
      .filter((_, i) => i % step === 0)
      .slice(0, PROMPT_MATURITY_SAMPLE_SIZE);
    ...
    const graded = sampled.map((r) => {
      ...
      return {
        text: r.messageText.substring(0, 150),
        grade: scoreToGrade(g.total),
        issues: g.issues,
        total: g.total,
    ...
    const overall = Math.round(
      (dims.constraints +
        dims.successCriteria +
        dims.verificationSteps +
        dims.contextProvision +
        dims.specificity) /
        5,
    );
  ...
  private getMigrationReadiness(f?: DateFilter): MigrationReadinessData {
    ...
    const sortedHarnesses = Array.from(harnessCounts.entries()).sort(
      (a, b) => b[1] - a[1],
    );
    ...
    const usedFeatures = this.getUsedMigrationFeatures(
      this.filter(f),
      sessions,
    );
    const missingFeatures = featureMatrix
      ...
      .map((feature) => ({
        ...
        availableIn: feature.harnesses.filter(
          (harness) =>
            harness !== primaryHarness && allHarnesses.includes(harness),
        ),
    ...
    const featureUsage = featureMatrix
      .filter((feature) =>
        feature.harnesses.some((harness) => allHarnesses.includes(harness)),
      )
      .map((feature) => ({
        feature: feature.feature,
        used: usedFeatures.has(feature.feature),
        harnesses: feature.harnesses.filter((harness) =>
          allHarnesses.includes(harness),
        ),
      }));
    const readinessScore =
      featureMatrix.length > 0
        ? Math.round((usedFeatures.size / featureMatrix.length) * 100)
        : 0;

    return { primaryHarness, missingFeatures, readinessScore, featureUsage };
  }

  private getReviewGap(
    current: Session["requests"][number],
    next: Session["requests"][number],
  ): number | null {
    if (!current.timestamp || !next.timestamp || !current.totalElapsed)
      return null;
  ...
  private getSustainablePaceAlerts(
    ...
    if (currentStreak >= BURNOUT_STREAK_DAYS) {
      alerts.push(
        `Working ${currentStreak} consecutive days without a break. Consider taking a day off.`,
      );
    }
    if (lateNightTrending === "increasing") {
      alerts.push(
        "Late-night sessions are increasing week over week. This is a burnout risk factor.",
      );
    }
    if (weekendTrending === "increasing") {
      alerts.push(
        "Weekend work is creeping up. Monitor your work-life boundaries.",
      );
    }

    const totalReqs = reqs.length;
    const lateNightRate =
      totalReqs > 0 ? lateNightReqs.reduce((a, b) => a + b, 0) / totalReqs : 0;
    const weekendRate =
      totalReqs > 0 ? weekendReqs.reduce((a, b) => a + b, 0) / totalReqs : 0;
    if (lateNightRate > BURNOUT_LATE_NIGHT_RATE) {
      alerts.push(
        `${(lateNightRate * 100).toFixed(0)}% of requests happen during late-night hours (${LATE_NIGHT_START}:00-${LATE_NIGHT_END}:00).`,
      );
    }
    if (weekendRate > BURNOUT_WEEKEND_RATE) {
      alerts.push(
        `${(weekendRate * 100).toFixed(0)}% of requests happen on weekends.`,
      );
    }
    return alerts;
  }
  ...
  private getBurnoutRisk(
    ...
    if (
      alerts.length >= 3 ||
      (currentStreak >= BURNOUT_STREAK_DAYS &&
        lateNightTrending === "increasing")
    ) {
      return "high";
    }
  ...
  private getMigrationFeatureMatrix(): Array<{
    feature: string;
    description: string;
    harnesses: string[];
  }> {
    return [
      {
        feature: "Sub-agents",
        description: "Delegate sub-tasks to specialized agents",
        harnesses: ["Local Agent", "Local Agent (Insiders)", "Claude"],
      },
      {
        feature: "MCP Tools",
      ...
      },
      {
        ...
        description: "Project-level AI instructions (.instructions.md)",
        harnesses: ["Local Agent", "Local Agent (Insiders)", "Claude"],
      },
      {
        feature: "Plan Mode",
        description: "Separate planning step before implementation",
        harnesses: ["Local Agent", "Local Agent (Insiders)", "Claude"],
      },
      {
        feature: "Skills",
        description: "Domain-specific knowledge modules",
        harnesses: ["Local Agent", "Local Agent (Insiders)"],
      },
      {
        feature: "Slash Commands",
        description: "/fix, /explain, /tests, /doc",
        harnesses: ["Local Agent", "Local Agent (Insiders)"],
      },
      {
        feature: "Multi-file Edits",
        description: "Edit multiple files in a single turn",
        harnesses: ["Local Agent", "Local Agent (Insiders)", "Claude", "Codex"],
      },
      {
        feature: "Terminal Access",
        description: "Run commands as part of agent workflow",
        harnesses: [
          "Local Agent",
      ...
      },
      {
        ...
        description: "Reference specific files in prompts",
        harnesses: ["Local Agent", "Local Agent (Insiders)", "Claude"],
      },
      {
        feature: "Parallel Sessions",
        description: "Run multiple conversations simultaneously",
        harnesses: ["Local Agent", "Local Agent (Insiders)", "Claude", "Codex"],
      },
    ];
  }

  private getUsedMigrationFeatures(
    reqs: Session["requests"],
    sessions: Session[],
  ): Set<string> {
    const usedFeatures = new Set<string>();
    for (const request of reqs) {
      if (request.agentName && request.agentName !== "copilot")
        usedFeatures.add("Sub-agents");
      if (request.toolsUsed.some((tool) => tool.startsWith("mcp_")))
        usedFeatures.add("MCP Tools");
      if (request.customInstructions.length > 0)
        usedFeatures.add("Custom Instructions");
      if (request.agentMode.includes("plan") || request.slashCommand === "plan")
        usedFeatures.add("Plan Mode");
      ...
      if (
        request.toolsUsed.some(
          (tool) => tool.includes("terminal") || tool.includes("runCommand"),
        )
      )
    ...
    if (Array.from(daySessions.values()).some((count) => count >= 2))
      usedFeatures.add("Parallel Sessions");
    return usedFeatures;
  }
}
