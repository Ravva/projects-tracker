...
export class PatternsAnalyzer extends AnalyzerBase {
  ...
  private checkModelDiversity(reqs: SessionRequest[]): RecommendationResult {
    ...
    return {
      ...
      recommendation:
        count < 3
          ? "Try lighter models (gpt-4.1-mini, gemini-flash) for simple tasks to save premium requests."
          : "Good model diversity.",
    };
  }

  private checkModelTaskAlignment(
    reqs: SessionRequest[],
  ): RecommendationResult {
    let aligned = 0,
      total = 0;
    ...
    return {
      checkId: "model-task-align",
      name: "Model-Task Alignment",
      category: "Efficiency",
      score,
      status: scoreToStatus(score),
      ...
      recommendation:
        score < 70
          ? "Use lightweight models for simple questions and heavy models for complex coding tasks."
          : "Good alignment between model size and task complexity.",
    };
  }

  private checkPlanningUsage(reqs: SessionRequest[]): RecommendationResult {
    const planningReqs = reqs.filter(
      (r) =>
    ...
    return {
      checkId: "planning-mode",
      name: "Planning-First Usage",
      category: "Strategy",
      score,
      status: scoreToStatus(score),
      ...
      recommendation:
        score < 70
          ? "Use planning mode before large tasks to help Copilot understand the big picture."
  ...
  private checkSessionLengthHygiene(sessions: Session[]): RecommendationResult {
    const longSessions = sessions.filter(
      (s) => s.requestCount > LONG_SESSION_REQS,
    );
    const ratio =
      sessions.length > 0 ? longSessions.length / sessions.length : 0;
    const score = ratio < 0.1 ? 100 : ratio < 0.3 ? 60 : 20;
    return {
      checkId: "context-flush",
      name: "Session Length Hygiene",
      category: "Context",
      score,
      status: scoreToStatus(score),
      ...
      recommendation:
        score < 70
          ? "Start new sessions periodically to avoid context window pollution. Long sessions degrade response quality."
          : "Good session hygiene.",
    };
  }

  private checkSlashCommandAdoption(
    reqs: SessionRequest[],
  ): RecommendationResult {
    ...
    return {
      checkId: "slash-commands",
      name: "Slash Command Adoption",
      category: "Features",
      score,
      status: scoreToStatus(score),
      ...
      recommendation:
        score < 70
          ? "Try /fix, /explain, /tests, /doc to get more targeted responses."
          : "Good slash command usage.",
    };
  }
  ...
  private checkFeatureBreadth(reqs: SessionRequest[]): RecommendationResult {
    ...
    return {
      ...
      recommendation:
        score < 70
          ? "Explore more features: agents, slash commands, file references, custom instructions, skills."
          : "Great feature breadth.",
    };
  }
  ...
  private checkParallelism(sessions: Session[]): RecommendationResult {
    ...
    const avgParallel =
      daySessionCounts.size > 0
        ? Array.from(daySessionCounts.values()).reduce((a, b) => a + b, 0) /
          daySessionCounts.size
        : 0;
    const score =
      avgParallel >= 3
        ? 100
    ...
    return {
      ...
      recommendation:
        score < 70
          ? "Try running multiple Copilot sessions to keep work moving while waiting for responses."
          : "Good parallel session usage.",
    };
  }
    ...
    return {
      ...
      recommendation:
        score < 70
          ...
          : "Low cancellation rate, good prompt clarity.",
    };
  }
  ...
  private checkToolDiversity(reqs: SessionRequest[]): RecommendationResult {
    ...
    return {
      ...
      recommendation:
        score < 70
          ? "Explore more tools in agentic mode: terminal, file search, web search, etc."
          : "Good tool diversity.",
    };
  }

  private checkResponseEfficiency(
    reqs: SessionRequest[],
  ): RecommendationResult {
    const withTiming = reqs.filter(
      (r) => r.totalElapsed != null && r.totalElapsed > 0,
    );
    const avgTime =
      withTiming.length > 0
        ? withTiming.reduce((s, r) => s + r.totalElapsed!, 0) /
          withTiming.length
        : 0;
    ...
    return {
      ...
      recommendation:
        score < 70
          ? "Long responses may indicate overly broad prompts. Break complex tasks into smaller requests."
          : "Good response times.",
    };
  }
  ...
  private checkFileContextUsage(reqs: SessionRequest[]): RecommendationResult {
    ...
    const score =
      ratio > 0.3 ? 100 : ratio > 0.15 ? 70 : ratio > 0.05 ? 40 : 10;
    return {
      checkId: "file-refs",
      name: "File Context Usage",
      category: "Context",
      score,
      status: scoreToStatus(score),
      ...
      recommendation:
        score < 70
          ? "Reference files with #file to give Copilot better context about your codebase."
          : "Good use of file context.",
    };
  }

  private checkSessionSizeDistribution(
    sessions: Session[],
  ): RecommendationResult {
    ...
    const score =
      avg >= 3 && avg <= 15 && ratio < 0.3
        ? 100
        : avg >= 2 && avg <= 25
          ? 60
          : 30;
    return {
      checkId: "session-length",
      name: "Session Size Distribution",
      category: "Context",
      score,
      status: scoreToStatus(score),
      ...
      recommendation:
        score < 70
          ? "Many single-request sessions? Consider using follow-up questions to refine responses."
          : "Healthy session size distribution.",
    };
  }

  /** Public access to filtered requests (for rule editor). */
  getFilteredRequests(f?: DateFilter): SessionRequest[] {
    return this.filter(f);
  }
  /** Public access to filtered sessions (for rule editor). */
  getFilteredSessions(f?: DateFilter): Session[] {
    return this.filteredSessions(f);
  }
  ...
  getAntiPatterns(f?: DateFilter): AntiPatternData {
    ...
    const enrichedReqs = reqs.map((r) => {
      ...
      if (!s) return r;
      const enriched = r as SessionRequest & {
        sessionId: string;
    ...
    const skipIdeDetectors = !!(
      f?.harness &&
      !f.harness.startsWith("Local Agent") &&
      f.harness !== "Xcode"
    );
  ...
  private buildAntiPatternResult(
    patterns: import("./types").AntiPattern[],
    reqs: SessionRequest[],
    ...
    const groupScores: GroupScore[] = allGroups.map((group) => {
      ...
      if (series && series.scores.length >= 5) {
        ...
        if (prevMonth.length > 0) {
          const avgRecent =
            recentMonth.reduce((a, b) => a + b, 0) / recentMonth.length;
          const avgPrev =
            prevMonth.reduce((a, b) => a + b, 0) / prevMonth.length;
          momPct =
            avgPrev > 0
  ...
  getProjectOverview(f?: DateFilter): ProjectOverviewData {
    ...
    const projects = Array.from(
      this.groupWorkspaceRequests(sessions, f).entries(),
    )
      .map(([workspaceName, data]) =>
        this.buildProjectOverviewItem(workspaceName, data),
      )
  ...
  private groupWorkspaceRequests(
    ...
    const wsMap = new Map<
      string,
      { id: string; sessions: Session[]; reqs: SessionRequest[] }
    >();
    for (const session of sessions) {
      if (!wsMap.has(session.workspaceName)) {
        wsMap.set(session.workspaceName, {
  ...
  private getProjectLanguages(reqs: SessionRequest[]): string[] {
    ...
    for (const request of reqs) {
      for (const codeBlock of [...request.aiCode, ...request.userCode]) {
        ...
        langCounts.set(
          language,
          (langCounts.get(language) || 0) + codeBlock.loc,
        );
      }
