...
function stripByteArrayImages(raw: string): string {
  ...
  if (!raw.includes("[255,216,255") && !raw.includes("[137,80,78,71"))
    return raw;

  // Match "data":[<JPEG or PNG signature>,...] — the array can be very large
  return raw.replaceAll(
    /"data":\[(255,216,255|137,80,78,71)[0-9,]*\]/g,
    '"data":[]',
  );
}
...
function appendAtPath(obj: JsonValue, keys: PathKey[], items: JsonValue): void {
  ...
  for (const key of keys) {
    if (typeof key === "number" && Array.isArray(target)) {
      ...
      if (!((key as string) in target))
        (target as JsonObject)[key as string] = [];
...
export function reconstructFromJsonl(
  fpath: string,
  ...
  for (const line of lines) {
    ...
    if (!trimmed) continue;
    try {
      const entry = JSON.parse(stripImageData(trimmed)) as {
        kind: number;
        k?: PathKey[];
        v?: JsonValue;
      };
  ...
  for (const line of raw.split("\n")) {
    ...
    if (entry.kind === 0) {
      ...
      for (const req of state.requests || []) {
        if (req.requestId !== requestId) continue;
        return extractImagesFromVariables(
          req.variableData?.variables || [],
        ).slice(0, 4);
      }
    } else if (entry.kind === 2) {
...
export function parseWorkspaceName(wsJsonPath: string): string {
  try {
    ...
    debugCore(
      "parser-vscode",
      `Could not parse workspace name from ${wsJsonPath}`,
      e,
    );
    return "unknown";
  }
...
export function parseWorkspaceFolderPath(wsJsonPath: string): string | null {
  try {
    ...
    debugCore(
      "parser-vscode",
      `Could not parse workspace folder path from ${wsJsonPath}`,
      e,
    );
    return null;
  }
...
export function parseCLIWorkspaceName(wsYamlPath: string): string {
  try {
    ...
    debugCore(
      "parser-vscode",
      `Could not parse CLI workspace name from ${wsYamlPath}`,
      e,
    );
    return "unknown";
  }
...
export function parseCLIWorkspaceFolderPath(wsYamlPath: string): string | null {
  try {
    ...
    debugCore(
      "parser-vscode",
      `Could not parse CLI workspace cwd from ${wsYamlPath}`,
      e,
    );
    return null;
  }
