...
export function parseInMemoryOpenCodeSessions(
  files: { path: string; content: string }[],
): Session[] {
  const sessions: Session[] = [];

  const readAllJsonInMemory = <T>(subPath: string): T[] => {
    const results: T[] = [];
    for (const f of files) {
      const normPath = f.path.replace(/\\/g, "/");
      if (normPath.includes(subPath) && normPath.endsWith(".json")) {
        try {
          results.push(JSON.parse(f.content) as T);
        } catch {
          // ignore malformed
        }
      }
  ...
  const rawSessions = readAllJsonInMemory<OcSession>("session/global");

  for (const rawSession of rawSessions) {
    if (!rawSession.id) continue;

    const rawMessages = readAllJsonInMemory<OcMessage>(
      `message/${rawSession.id}`,
    );
    rawMessages.sort((a, b) => (a.time?.created || 0) - (b.time?.created || 0));
    ...
    for (const msg of rawMessages) {
      if (!msg.id) continue;
      const parts = readAllJsonInMemory<OcPart>(`part/${msg.id}`);
      partsByMsg.set(msg.id, parts);
    }
    ...
    for (let i = 0; i < rawMessages.length; i++) {
      ...
      lastTs = assistantData.lastTs;
      requests.push(
        buildOpenCodeRequest(msg, partsByMsg, assistantData, userTs),
      );
    }
