...
function dirFingerprint(
  ...
  try {
    ...
    const count = countDirs
      ...
      : entries.filter((n) => n.endsWith(".json") || n.endsWith(".jsonl"))
          .length;
 ...
 * Uses directory mtime (changes on add/remove) + count of relevant entries.
 */
async function dirFingerprintAsync(
  dirPath: string,
  countDirs = false,
): Promise<{ count: number; mtime: number }> {
  try {
...
function readSerializedCachePayload(
  value: unknown,
): SerializedCachePayload | null {
  if (!isRecord(value)) return null;
  if (
    !Array.isArray(value.workspaces) ||
    !Array.isArray(value.sessions) ||
    !Array.isArray(value.editLocIndex)
  ) {
    return null;
  }
  return {
    ...
    editLocIndex: value.editLocIndex as Array<
...
export function computeDirMetas(logsDirs: string[]): DirMetas {
  ...
  for (const logsDir of logsDirs) {
    ...
    if (
      logsDir.includes(path.join(".config", "github-copilot", "xcode")) ||
 ...
 */
export async function computeDirMetasAsync(
  logsDirs: string[],
): Promise<DirMetas> {
  const metas: DirMetas = {};
...
export async function loadCacheData(): Promise<CacheData | null> {
  try {
    ...
    const meta = readCacheMetaPayload(
      JSON.parse(fs.readFileSync(CACHE_META, "utf-8")) as unknown,
    );
...
export function saveCacheData(result: ParseResult, dirMetas: DirMetas): void {
  ...
  try {
    ...
    const serializable = {
      ...
      editLocIndex: Array.from(result.editLocIndex.entries()).map(([k, v]) => [
        k,
    ...
    void import("node:worker_threads")
      .then(({ Worker }) => {
        try {
          ...
          w.on("error", (e: Error) =>
            console.warn("[cache] worker write error:", e.message),
          );
          w.unref();
        } catch {
          fallbackWrite();
...
export function loadSidebarStats(): SidebarStats | null {
  try {
    ...
    const raw = JSON.parse(
      fs.readFileSync(SIDEBAR_STATS_FILE, "utf-8"),
    ) as Partial<SidebarStats>;
    // Reject malformed data missing required fields
    if (!Array.isArray(raw.harnesses) || typeof raw.savedAt !== "number")
      return null;
...
export function clearCache(): void {
  ...
  try {
    ...
    console.debug(
      "Cache file removal skipped:",
      e instanceof Error ? e.message : e,
    );
  }
  try {
    fs.unlinkSync(CACHE_META);
  } catch (e) {
    console.debug(
      "Cache meta removal skipped:",
      e instanceof Error ? e.message : e,
    );
  }
  try {
    fs.unlinkSync(SIDEBAR_STATS_FILE);
  } catch (e) {
    console.debug(
      "Sidebar stats removal skipped:",
      e instanceof Error ? e.message : e,
    );
  }
}
