...
const KNOWN_FILES: FilePattern[] = [
  { relativePath: ".github/copilot-instructions.md", kind: "instruction" },
  {
    ...
    kind: "instruction",
    isDir: true,
    dirGlob: /\.instructions\.md$/i,
  },
  {
    relativePath: ".github/prompts",
    kind: "prompt",
    isDir: true,
    dirGlob: /\.prompt\.md$/i,
    recurse: true,
  ...
  {
    relativePath: ".github/agents",
    kind: "agent",
    isDir: true,
    dirGlob: /\.md$/i,
    recurse: true,
    fileValidator: isAgentProfileFile,
  },
  {
    relativePath: "agents",
    kind: "agent",
...
function resolveClaudeRoot(projectDir: string): string | null {
  try {
    ...
    const firstLine = fs
      .readFileSync(path.join(projectDir, files[0]), "utf-8")
      .split("\n")[0];
...
export function scanConfigFiles(rootPath: string): ConfigFileInfo[] {
  ...
  const rootIsHome =
    home !== "" && path.resolve(rootPath) === path.resolve(home);
...
function scanFlatDirectoryEntry(
  fullPath: string,
  pattern: FilePattern,
  entry: string,
): ConfigFileInfo | null {
  ...
  if (pattern.fileValidator && !pattern.fileValidator(filePath, fileRelative))
    return null;
  return analyzeFile(filePath, fileRelative, pattern.kind);
}

function scanSingleFilePattern(
  fullPath: string,
  pattern: FilePattern,
): ConfigFileInfo | null {
  try {
...
function analyzeFile(
  ...
  const markdownIssues = isMarkdown
    ? checkMarkdownQuality(content, relativePath)
    : [];
  ...
  if (
    ...
    chars > COPILOT_INSTRUCTION_MAX_CHARS
  ) {
    sizeVerdict = "oversized";
  }
  if (
    (relativePath === "CLAUDE.md" || relativePath === "CLAUDE.local.md") &&
    lines > CLAUDE_MD_RECOMMENDED_LINES
  ) {
    sizeVerdict = "oversized";
  }
...
function checkMarkdownQuality(content: string, relativePath: string): string[] {
  ...
  if (
    ...
    issues.push(
      "File should start with a markdown heading (# Title) or YAML frontmatter (---)",
    );
  }
  ...
  if (lines.length > 30 && headingCount < 2) {
    issues.push(
      "Long file lacks section headings -- use ## sections to organize instructions",
    );
  }
  ...
  const imperativeRe =
    /^[-*]?\s*(Use|Do|Don't|Always|Never|Ensure|Check|Avoid|Prefer|Write|Follow|Include|Exclude|Run|Test|Create|Add|Remove|Set|Keep|Limit|Apply)/i;
  const imperativeCount = instructionLines.filter((l) =>
    imperativeRe.test(l.trim()),
  ).length;
  if (instructionLines.length > 5 && imperativeCount === 0) {
    issues.push(
      'Instructions should use imperative phrasing (e.g., "Use TypeScript", "Avoid class components")',
    );
  }

  const conditionalCount = lines.filter(
    (l) => /\bif\b.*\bthen\b/i.test(l) || /\bwhen\b.*\bdo\b/i.test(l),
  ).length;
  if (conditionalCount >= 3) {
    issues.push(
      `${conditionalCount} conditional if/then chains detected -- LLMs handle flat, imperative rules better than nested conditions`,
    );
  }
  ...
  if (fenceOpens % 2 !== 0) {
    issues.push(
      "Unclosed code block (```) detected -- ensure all code fences are properly paired",
    );
  }

  if (
    relativePath === ".github/copilot-instructions.md" &&
    content.length > COPILOT_INSTRUCTION_MAX_CHARS
  ) {
    issues.push(
      `File exceeds ${COPILOT_INSTRUCTION_MAX_CHARS} characters -- Copilot code review truncates beyond this limit`,
    );
  }

  if (
    (relativePath === "CLAUDE.md" || relativePath === "CLAUDE.local.md") &&
    lines.length > CLAUDE_MD_RECOMMENDED_LINES
  ) {
    issues.push(
      `File exceeds ${CLAUDE_MD_RECOMMENDED_LINES} lines -- keep CLAUDE.md lean and use @import for domain-specific rules`,
    );
  }
...
export function computeProgressiveDisclosureScore(
  files: ConfigFileInfo[],
): number {
  let score = 0;

  const hasInstructions = files.some(
    (f) => f.kind === "instruction" || f.kind === "claude-md",
  );
  ...
  const allCompact =
    ...
    instructionFiles.every((f) => f.sizeVerdict !== "oversized");
  if (allCompact && instructionFiles.length > 0) score += 25;
  else if (instructionFiles.some((f) => f.sizeVerdict === "moderate"))
    score += 10;

  const hasSkills = files.some((f) => f.kind === "skill");
  if (hasSkills) score += 25;

  const scopedInstructionFiles = files.filter(
    (f) => f.kind === "instruction" && f.relativePath.includes("instructions/"),
  );
  const hasPrompts = files.some((f) => f.kind === "prompt");
  const hasAgents = files.some((f) => f.kind === "agent");
  if (
    scopedInstructionFiles.length >= 2 ||
    (hasSkills && hasPrompts) ||
    hasAgents
  )
    score += 25;
...
export function computeInstructionQualityScore(
  files: ConfigFileInfo[],
): number {
...
export function generateWorkspaceSuggestions(
  ...
  const hasAnyInstructions = files.some(
    (f) => f.kind === "instruction" || f.kind === "claude-md",
  );
  if (!hasAnyInstructions) {
    suggestions.push(
      "Create a .github/copilot-instructions.md file with project conventions -- this is the single most impactful context file for GitHub Copilot.",
    );
  }
  ...
  for (const f of files) {
    if (f.sizeVerdict === "oversized") {
      if (f.kind === "instruction" && f.lines > OVERSIZED_INSTRUCTION_LINES) {
        suggestions.push(
          `${f.relativePath} has ${f.lines} lines -- split domain-specific rules into .github/instructions/*.instructions.md files or .github/skills/*/SKILL.md for progressive disclosure.`,
        );
      } else if (f.kind === "claude-md") {
        suggestions.push(
          `${f.relativePath} has ${f.lines} lines (recommended: <${CLAUDE_MD_RECOMMENDED_LINES}). Use @import syntax to load domain-specific rules from sub-files.`,
        );
      } else if (
        f.relativePath === ".github/copilot-instructions.md" &&
        f.chars > COPILOT_INSTRUCTION_MAX_CHARS
      ) {
        suggestions.push(
          `${f.relativePath} exceeds ${COPILOT_INSTRUCTION_MAX_CHARS} chars -- Copilot code review silently truncates beyond this. Move scoped rules to .github/instructions/*.instructions.md.`,
        );
      }
    }
  }
  ...
  for (const f of files) {
    if (f.markdownIssues.length > 0)
      suggestions.push(`${f.relativePath}: ${f.markdownIssues[0]}`);
  }

  const hasSkills = files.some((f) => f.kind === "skill");
  if (
    !hasSkills &&
    files.some((f) => f.kind === "instruction" && f.lines > 100)
  ) {
    suggestions.push(
      "Consider extracting domain-specific knowledge into .github/skills/*/SKILL.md files. Skills use progressive disclosure: only the name/description is loaded initially, full instructions load only when matched.",
    );
  }
  ...
  if (!hasPrompts) {
    suggestions.push(
      "Save reusable prompts as .github/prompts/*.prompt.md files. These can be invoked with /promptName in chat.",
    );
  }

  if (isClaudeWorkspace) {
    const hasClaudeMd = files.some(
      (f) => f.kind === "claude-md" && f.relativePath === "CLAUDE.md",
    );
    if (!hasClaudeMd) {
      suggestions.push(
        "Create a CLAUDE.md file for Claude Code instructions. Focus on deviations from standard practices -- Claude already knows common conventions.",
      );
    }
    if (!hookCoverage) {
      suggestions.push(
        "No hooks configured in .claude/settings.json. Hooks enforce deterministic boundaries: PreToolUse for security (block sensitive file edits), PostToolUse for auto-formatting (Prettier, Ruff).",
      );
    } else {
      if (!hookCoverage.hasPreToolUse)
        suggestions.push(
          "Add a PreToolUse hook to enforce security boundaries (e.g., block edits to .env, migrations/, or .git/ files).",
        );
      if (!hookCoverage.hasPostToolUse)
        suggestions.push(
          "Add a PostToolUse hook for auto-formatting (run Prettier/Ruff after file writes) and audit logging.",
        );
    }
  }
