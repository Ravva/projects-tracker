...
function computeBilling(
  sessions: Session[],
): Map<SessionRequest, RequestBilling> {
  ...
  for (const session of sessions) {
    if (session.modelUsage) {
      ...
      const isPending =
        session.endReason === "active" || session.endReason === "aborted";
      ...
      const preferPending =
        isPending && harnessUsesSessionAggregated(session.harness);
...
function attributeSessionLevel(
  ...
  for (const [model, reqs] of byModel) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      const credits = tokenCostInCredits(
        model,
        uncachedInput,
        output,
        cacheRead,
...
export class ConsumptionAnalyzer extends AnalyzerBase {
  getConsumption(f?: DateFilter): ConsumptionData {
    ...
    const dayCount =
      new Set(reqs.map((r) => toDateStr(r.timestamp!))).size || 1;
    const weekCount = weeklyMap.size || 1;
    const monthCount = monthlyMap.size || 1;

    const allModels = Array.from(modelTotals.keys()).sort(
  ...
  getAiCredits(f?: DateFilter): AiCreditData {
    ...
    summary.topRequests.sort(
      (a, b) =>
        b.inputTokens + b.outputTokens - (a.inputTokens + a.outputTokens),
    );
    summary.topRequests.splice(10);
    const allModels = Array.from(summary.costByModel.keys()).sort(
      (a, b) =>
        (summary.costByModel.get(b)?.credits || 0) -
        (summary.costByModel.get(a)?.credits || 0),
    );
    const countedDays = new Set(
      reqs
        .filter((request) => billing.get(request)?.status === "complete")
        .map((request) => toDateStr(request.timestamp!)),
    );
    ...
    return {
      ...
      delegatedRequests: 0,
      finalizableRequests:
        reqs.length - summary.pendingCount - summary.noDataCount,
      missingPct: computeMissingPct(
        reqs.length,
      ...
      avgCreditsPerRequest:
        summary.countedCount > 0
          ? Math.round((summary.totalCredits / summary.countedCount) * 100) /
            100
          : 0,
      avgCreditsPerDay:
        Math.round(
          (summary.totalCredits / Math.max(countedDays.size, 1)) * 100,
        ) / 100,
      costByModel: this.buildAiCreditCostByModel(summary.costByModel),
      daily: this.buildAiCreditSeries(summary.dailyMap, allModels, (keys) =>
        fillDayRange(this.anchorFromDate(keys, f)),
      ),
      weekly: this.buildAiCreditSeries(
        summary.weeklyMap,
        allModels,
        fillWeekRange,
      ),
      dailyTokensByWorkspace: this.buildDailyTokensByWorkspace(
        summary.dailyTokensByWorkspace,
        (keys) => fillDayRange(this.anchorFromDate(keys, f)),
      ),
      dailyTokensByHarness: this.buildDailyTokensByHarness(
        summary.dailyTokensByHarness,
        (keys) => fillDayRange(this.anchorFromDate(keys, f)),
      ),
  ...
  private applyAiCreditRequest(
    ...
    summary.topRequests.push({
      ...
      outputTokens: Math.round(
        resolvedBilling.status === "partial"
          ? resolvedBilling.partialOutput
          : resolvedBilling.output,
      ),
  ...
  private addAiCreditSeriesPoint(
    ...
    for (const [mapRef, key] of [
      [summary.dailyMap, day],
      [summary.weeklyMap, week],
    ] as const) {
      if (!mapRef.has(key)) mapRef.set(key, new Map());
      const inner = mapRef.get(key)!;
    ...
    if (!summary.dailyTokensByWorkspace.has(ws))
      summary.dailyTokensByWorkspace.set(ws, new Map());
    ...
    if (!summary.dailyTokensByHarness.has(harness))
      summary.dailyTokensByHarness.set(harness, new Map());
  ...
  private buildAiCreditSeries(
    ...
    for (const label of labels) {
      ...
      let dayTotal = 0;
      if (inner) {
        for (const value of inner.values()) dayTotal += value;
      }
  ...
  private buildAiCreditCostByModel(
    ...
    for (const [model, entry] of costByModel) {
      const finalizableRequests =
        entry.requests - entry.pendingRequests - entry.noDataRequests;
      out[model] = {
  ...
  getAiCreditBurndown(
    ...
    const budget =
      config.customBudget ??
      Object.values(modelBudgetsIn).reduce((a, b) => a + b, 0);
    const now = new Date();
    const targetMonth =
      config.month ||
      `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, "0")}`;
    ...
    const monthEnd = `${targetMonth}-${String(daysInMonth).padStart(2, "0")}`;
    const reqs = this.filter({ ...f, fromDate: monthStart, toDate: monthEnd });
    const burndown = this.buildAiCreditBurndownData(
      reqs,
      computeBilling(this.sessions),
      daysInMonth,
    );
    const series = this.buildAiCreditBurndownSeries(
      burndown.dailyTokens,
      daysInMonth,
    );
    const consumed = Math.round(series.total);
    const dailyRate = dayOfMonth > 0 ? consumed / dayOfMonth : 0;
    const projected = Math.round(dailyRate * daysInMonth);
    const projectedLine = series.labels.map((_, i) =>
      Math.round(dailyRate * (i + 1)),
    );
    ...
    const safeDailyBudget =
      budget > 0 && daysInMonth > dayOfMonth
        ? Math.round(remaining / (daysInMonth - dayOfMonth))
        : 0;
    const daysUntilExhaustion =
      budget > 0 && dailyRate > 0 ? Math.round(remaining / dailyRate) : null;
    const projectedOverage = budget > 0 ? Math.max(0, projected - budget) : 0;
    const missingPct = computeMissingPct(
      reqs.length,
      burndown.countedCount,
      burndown.partialCount,
      burndown.pendingCount,
      burndown.noDataCount,
    );
    ...
    return {
      ...
      delegatedRequests: 0,
      finalizableRequests:
        reqs.length - burndown.pendingCount - burndown.noDataCount,
  ...
  private buildAiCreditBurndownData(
    ...
    for (const request of reqs) {
      ...
      if (status === "complete") {
        countedCount++;
        const tokens =
          (requestBilling?.totalInput ?? 0) + (requestBilling?.output ?? 0);
        dailyTokens.set(
          dayIdx + 1,
          (dailyTokens.get(dayIdx + 1) || 0) + tokens,
        );
        const model = normalizeModel(request.modelId ?? "unknown");
        if (!dailyTokensByModel.has(model))
          dailyTokensByModel.set(model, new Map());
  ...
  private getAiCreditBurndownRecommendation(params: {
    ...
    const partialNote =
      params.missingPct > 0 && params.missingPct < 100
    ...
    if (params.countedCount === 0) {
      if (params.totalRequests === 0) {
        return {
          status: "no-data",
          recommendation: "No requests in this period.",
        };
      }
  ...
  getTokenCoverage(f?: DateFilter): TokenCoverageData {
    ...
    const aggregates = this.buildTokenCoverageAggregates(
      reqs,
      computeBilling(this.sessions),
      sessionByRequest,
    );
    ...
    return {
      ...
      totalWorkspaces: new Set(
        Array.from(
          aggregates.inWindowSessions,
          (s) => s.workspaceId || s.workspaceName || "unknown",
        ),
      ).size,
      ...
      missingPct: this.computeOverallMissingPct(
        reqs.length,
        aggregates.pendingRequests,
        aggregates.noDataRequests,
        aggregates.missingRequests,
      ),
      ...
      timeline: this.buildTokenCoverageTimeline(
        aggregates.timelineMap,
        aggregates.harnessMap,
      ),
    };
  }
  ...
  private buildTokenCoverageAggregates(
    ...
    for (const request of reqs) {
      ...
      const requestBilling = billing.get(request);
      const status = requestBilling?.status ?? "missing";
      const harnessEntry = this.getTokenCoverageHarnessEntry(
        harnessMap,
        harness,
        session,
        wsKey,
      );
      const workspaceEntry = this.getTokenCoverageWorkspaceEntry(
        workspaceMap,
        wsKey,
        session,
        harness,
      );
      const sessionEntry = this.getTokenCoverageSessionEntry(
        sessionMap,
        session,
      );
      this.updateTokenCoverageTimeline(
        timelineMap,
        harness,
        request.timestamp,
        status,
  ...
  private getTokenCoverageWorkspaceEntry(
    ...
    if (!entry) {
      entry = {
        workspaceId: session.workspaceId || wsKey,
        workspaceName: session.workspaceName || wsKey,
        sessions: new Set(),
        harnesses: new Set(),
        requests: 0,
        counted: 0,
  ...
  private getTokenCoverageSource(
    hasCounted: number,
    hasPartial: number,
    hasExact: boolean,
    hasAggregated: boolean,
  ): TokenCoverageHarness["source"] {
  ...
  private buildTokenCoverageByHarness(
    ...
    return Array.from(harnessMap.entries())
      .map(([harness, entry]) => ({
        ...
        source: this.getTokenCoverageSource(
          entry.counted,
          entry.partial,
          entry.hasExact,
          entry.hasAggregated,
  ...
  private buildTokenCoverageTimeline(
    ...
    const months = Array.from(
      new Set(
        Array.from(timelineMap.values()).flatMap((monthMap) => [
          ...monthMap.keys(),
        ]),
      ),
    ).sort();
    ...
    const cells: Record<
      string,
      Record<string, TokenCoverageTimelineCell | undefined>
    > = {};
  ...
  private computeOverallMissingPct(
    totalRequests: number,
    pendingRequests: number,
    noDataRequests: number,
    missingRequests: number,
    ...
    return finalizableTotal > 0
      ? Math.round((missingRequests / finalizableTotal) * 100)
      : 0;
  }
}
