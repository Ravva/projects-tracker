...
export class Analyzer {
  ...
  constructor(
    ...
    this.config = new ConfigAnalyzer(
      sessions,
      elIdx,
      this.workspaces,
      sharedMap,
  ...
  private addPatterns(
    target: AntiPatternData,
    patterns: AntiPatternData["patterns"],
  ): void {
    if (patterns.length === 0) return;
    target.patterns.push(...patterns);
    target.totalOccurrences += patterns.reduce(
      (sum, pattern) => sum + pattern.occurrences,
      0,
    );
  }

  private addPatternsSafely(
    target: AntiPatternData,
    read: () => AntiPatternData["patterns"],
  ): void {
    try {
  ...
  async warmUp(
    ...
    if (result) {
      if (result.antiPatterns)
        this.cache.set("getAntiPatterns", result.antiPatterns);
      if (result.configHealth)
        this.cache.set("getConfigHealth", result.configHealth);
    } else {
      this.warmUpSync();
    }
  ...
  private async warmUpViaWorker(): Promise<{
    ...
    return new Promise((resolve, reject) => {
      ...
      worker.on(
        ...
        (msg: {
          ...
          if (msg.type === "result") {
            resolve({
              antiPatterns: msg.antiPatterns ?? null,
              configHealth: msg.configHealth ?? null,
            });
          } else {
  ...
  getWorkspaces(): {
    ...
    return this.dashboard.getWorkspaces();
  }
  getHarnesses(): string[] {
    return this.dashboard.getHarnesses();
  }
  getHarnessBreakdown(f?: DateFilter): {
    labels: string[];
    sessions: number[];
    requests: number[];
  } {
    return this.dashboard.getHarnessBreakdown(f);
  }

  /** Public access to filtered requests for rule editor preview. */
  filterRequests(f?: DateFilter): SessionRequest[] {
    return this.patterns.getFilteredRequests(f);
  }
  /** Public access to filtered sessions for rule editor preview. */
  filterSessions(f?: DateFilter): Session[] {
    return this.patterns.getFilteredSessions(f);
  }
  getHarnessComparison(f?: DateFilter): HarnessComparisonData {
    return this.dashboard.getHarnessComparison(f);
  }
  getParserCoverage(): ParserCoverageData {
    return this.dashboard.getParserCoverage();
  }
  getParserPreview(focusField?: string): ParserPreviewData {
    return this.dashboard.getParserPreview(focusField);
  }
  getDailyActivity(f?: DateFilter): DailyActivity {
    return this.getCached("getDailyActivity", f, () =>
      this.dashboard.getDailyActivity(f),
    );
  }
  getWorkspaceBreakdown(f?: DateFilter): WorkspaceBreakdown {
    return this.getCached("getWorkspaceBreakdown", f, () =>
      this.dashboard.getWorkspaceBreakdown(f),
    );
  }
  getHourlyDistribution(f?: DateFilter): HourlyDistribution {
    return this.dashboard.getHourlyDistribution(f);
  }
  getHeatmap(f?: DateFilter): HeatmapData {
    return this.dashboard.getHeatmap(f);
  }

  getCodeProduction(f?: DateFilter): CodeProductionData {
    return this.production.getCodeProduction(f);
  }

  getConsumption(f?: DateFilter): ConsumptionData {
    return this.consumption.getConsumption(f);
  }
  getBurndown(config: BurndownConfig, f?: DateFilter): BurndownData {
    return this.consumption.getBurndown(config, f);
  }
  getAiCredits(f?: DateFilter): AiCreditData {
    return this.consumption.getAiCredits(f);
  }
  getAiCreditBurndown(
    config: BurndownConfig,
    f?: DateFilter,
  ): AiCreditBurndownData {
    return this.consumption.getAiCreditBurndown(config, f);
  }
  getTokenCoverage(f?: DateFilter): TokenCoverageData {
    return this.consumption.getTokenCoverage(f);
  }

  getDayTimeline(dateStr?: string, mode?: string, f?: DateFilter): DayTimeline {
    return this.timeline.getDayTimeline(dateStr, mode, f);
  }
  getSessions(
    page: number,
    pageSize: number,
    ...
    return this.timeline.getSessions(page, pageSize, f, search);
  }
  getSessionDetail(sessionId: string): Session | null {
    return this.timeline.getSessionDetail(sessionId);
  }
  getWorkLifeBalance(f?: DateFilter): WorkLifeBalanceResult | null {
    return this.timeline.getWorkLifeBalance(f);
  }

  getRecommendations(f?: DateFilter): RecommendationResult[] {
    return this.patterns.getRecommendations(f);
  }
  getProjectOverview(f?: DateFilter): ProjectOverviewData {
    return this.patterns.getProjectOverview(f);
  }
  getAntiPatterns(f?: DateFilter): AntiPatternData {
    if (!f && this.cache.has("getAntiPatterns"))
      return this.cache.get("getAntiPatterns") as AntiPatternData;
    const data = this.patterns.getAntiPatterns(f);
    this.addPatternsSafely(
      data,
      () => this.getConfigHealth(f).contextAntiPatterns,
    );
    this.addPatternsSafely(
      data,
      () => this.getContextManagement(f).antiPatterns,
    );
    return data;
  }

  getWorkflowOptimization(f?: DateFilter): WorkflowOptimizationData {
    return this.workflows.getWorkflowOptimization(f);
  }

  getConfigHealth(f?: DateFilter): ConfigHealthData {
    return this.getCached("getConfigHealth", f, () =>
      this.config.getConfigHealth(f),
    );
  }
  getContextReviewPayload(
    wsIds: string[],
  ): ReturnType<ConfigAnalyzer["getContextReviewPayload"]> {
    return this.config.getContextReviewPayload(wsIds);
  }
  getInsights(f?: DateFilter): InsightsData {
    return this.insights.getInsights(f);
  }

  getFlowState(f?: DateFilter): FlowStateData {
    return this.flow.getFlowState(f);
  }

  getContextManagement(f?: DateFilter): ContextManagementData {
    return this.context.getContextManagement(f);
  }
  getWorkspaceContextSessions(
    workspaceId: string,
    f?: DateFilter,
  ): ReturnType<ContextAnalyzer["getWorkspaceContextSessions"]> {
  ...
  getCalendarActivity(f?: DateFilter): CalendarActivityData {
    return this.dashboard.getCalendarActivity(f);
  }

  getImageGallery(f?: DateFilter): ImageGalleryData {
    return this.images.getImageGallery(f);
  }
