...
function computeSessionFlowScore(
  ...
  const rapidCount = followUpGaps.filter(
    (g) => g <= FLOW_RAPID_FOLLOWUP_SEC,
  ).length;
...
export class FlowAnalyzer extends AnalyzerBase {
  getFlowState(f?: DateFilter): FlowStateData {
    ...
    const avgFollowUpSec =
      allFollowUps.length > 0
        ? Math.round(
            allFollowUps.reduce((s, v) => s + v, 0) / allFollowUps.length,
          )
        : 0;
    const allBlocks = days.map((d) => d.longestBlockMin);
    const avgBlockMin =
      allBlocks.length > 0
        ? Math.round(allBlocks.reduce((s, v) => s + v, 0) / allBlocks.length)
        : 0;
    const overallFlowScore =
      days.length > 0
        ? Math.round(days.reduce((s, d) => s + d.avgFlowScore, 0) / days.length)
        : 0;
    const weeklyTrend = this.buildWeeklyTrend(days);
    const hourlyFlow = this.buildHourlyFlow(flowSessions);
    const suggestions = generateFlowSuggestions(
      days,
      flowSessions,
      hourlyFlow,
      overallFlowScore,
  ...
  private buildFlowSession(
    ...
    const rapidFollowUpRate =
      followUpGaps.length > 0
        ? followUpGaps.filter((g) => g <= FLOW_RAPID_FOLLOWUP_SEC).length /
          followUpGaps.length
        : 0;
    const flowScore = computeSessionFlowScore(
      followUpGaps,
      durationMin,
      timedReqs.length,
    );
  ...
  private buildFlowDays(flowSessions: FlowSession[]): FlowDay[] {
    ...
    const days = Array.from(dayMap.entries()).map(([date, daySessions]) => {
      ...
      const longestBlockMin = Math.round(
        Math.max(...blocks.map((b) => (b.end - b.start) / 60_000)),
      );
      const totalHours =
        Math.round(
      ...
      const avgFlowScore = Math.round(
        daySessions.reduce(
          (sum, flowSession) => sum + flowSession.flowScore,
          0,
        ) / daySessions.length,
      );
  ...
  private buildHourlyFlow(flowSessions: FlowSession[]): number[] {
    ...
    return hourTotals.map((total, index) =>
      hourCounts[index] > 0 ? Math.round(total / hourCounts[index]) : 0,
    );
  }
}
...
function generateFlowSuggestions(
  ...
  if (peakScore > 0) {
    suggestions.push(
      `Your deepest flow tends to happen around ${peakHour}:00 (score ${peakScore}). Block this time for uninterrupted work.`,
    );
  }

  // Short blocks
  const shortBlockDays = days.filter(
    (d) => d.longestBlockMin < 30 && d.blockCount > 2,
  );
  if (shortBlockDays.length > days.length * 0.3) {
    suggestions.push(
      `${shortBlockDays.length} days had no work block longer than 30 minutes. Meeting-heavy days fragment flow. Try batching meetings.`,
    );
  }
  ...
  if (fragmented.length > sessions.length * 0.3) {
    suggestions.push(
      `${fragmented.length} sessions were fragmented (long pauses between prompts). Close distractions — Slack, email, tabs — during coding sessions.`,
    );
  }
  ...
  if (slowSessions.length > sessions.length * 0.2) {
    suggestions.push(
      `${slowSessions.length} sessions had >2 minute median follow-up time. Pre-plan your next prompt while the agent works.`,
    );
  }
  ...
  if (overallScore < FLOW_MODERATE_SCORE) {
    suggestions.push(
      "Your overall flow score is low. The #1 lever: protect 2-hour uninterrupted blocks. Even one per day transforms output.",
    );
  } else if (overallScore >= FLOW_DEEP_SCORE) {
    suggestions.push(
      "Strong flow patterns detected. Maintain your current work blocks and keep distractions at bay.",
    );
  }
  ...
  if (deepDays.length > 0 && days.length > 0) {
    ...
    if (deepPct < 30) {
      suggestions.push(
        `Only ${deepPct}% of your days reach deep flow. Aim for at least 3 deep-flow days per week.`,
      );
    }
  }
