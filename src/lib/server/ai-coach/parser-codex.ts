...
function computeModelUsage(
  ...
  if (
    !state.hasTokenData ||
    (state.curTotalInput <= 0 && state.curTotalOutput <= 0)
  )
    return undefined;
  const billingModel = state.turnModel || model || "untracked";
  const uncachedInput = Math.max(
    0,
    state.curTotalInput - state.curTotalCachedInput,
  );
  return {
    [billingModel]: {
...
function computeEndReason(
  ...
  const everyRequestEmpty = requests.every(
    (request) =>
...
function isTurnEmpty(state: CodexParseState): boolean {
  return (
    state.currentAssistantTexts.length === 0 &&
...
function flushCodexTurn(state: CodexParseState, defaultModel: string): void {
  ...
  const reqPromptTokens =
    state.hasTokenData && state.curTotalInput > state.prevTotalInput
      ? state.curTotalInput - state.prevTotalInput
      : null;
  const reqCompletionTokens =
    state.hasTokenData && state.curTotalOutput > state.prevTotalOutput
      ? state.curTotalOutput - state.prevTotalOutput
      : null;
  ...
  state.requests.push(
    createRequest({
      ...
      totalElapsed:
        state.turnStartTs && state.lastTs
          ? state.lastTs - state.turnStartTs
          : null,
      promptTokens: reqPromptTokens,
      completionTokens: reqCompletionTokens,
      reasoningEffort:
        state.turnEffort ??
        extractReasoningEffortFromModelId(state.turnModel || defaultModel),
    }),
  );
...
function handleTokenCountEvent(
  payload: Record<string, unknown>,
  state: CodexParseState,
): void {
  ...
  state.curTotalInput =
    typeof totalUsage.input_tokens === "number" ? totalUsage.input_tokens : 0;
  state.curTotalOutput =
    typeof totalUsage.output_tokens === "number" ? totalUsage.output_tokens : 0;
  state.curTotalCachedInput =
...
function handleUserResponseItem(
  payload: Record<string, unknown>,
  state: CodexParseState,
  ts: number | null,
  defaultModel: string,
  ...
  for (const item of extractContentItems(payload.content)) {
    if (item.type !== "input_text" || !item.text || item.text.startsWith("<"))
      continue;
...
function handleAssistantResponseItem(
  payload: Record<string, unknown>,
  state: CodexParseState,
): void {
  for (const item of extractContentItems(payload.content)) {
...
function handleFunctionCallResponseItem(
  payload: Record<string, unknown>,
  state: CodexParseState,
): void {
...
function handleResponseItem(
  ...
  if (itemType === "function_call")
    handleFunctionCallResponseItem(payload, state);
}
...
function parseCodexSessionFile(filePath: string): Session | null {
  ...
  for (const line of lines) {
    ...
    if (line.type === "response_item")
      handleResponseItem(line.payload || {}, state, ts, meta.model);
  }
