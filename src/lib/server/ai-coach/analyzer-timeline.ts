...
export class TimelineAnalyzer extends AnalyzerBase {
  getDayTimeline(dateStr?: string, mode?: string, f?: DateFilter): DayTimeline {
    ...
    const tlSessions = allSessions
      .map((session) =>
        this.buildTimelineSession(session, dayStartTs, dayEndTs),
      )
      .filter((session): session is TimelineSession => session !== null)
      .sort((a, b) => a.firstActivity - b.firstActivity);
    const { prevDay, nextDay, firstDay } = this.getDateNavigation(
      targetDate,
      sortedDates,
    );
    ...
    for (const session of sessions) {
      for (const request of session.requests) {
        if (request.timestamp != null && request.timestamp > 0)
          activeDates.add(toDateStr(request.timestamp));
      }
    }
  ...
  private buildTimelineSession(
    session: Session,
    dayStartTs: number,
    dayEndTs: number,
  ): TimelineSession | null {
    const dayReqs = session.requests.filter(
      (r) =>
        r.timestamp != null &&
        r.timestamp >= dayStartTs &&
        r.timestamp <= dayEndTs,
    );
  ...
  private getDateNavigation(
    dateStr: string,
    sortedDates: string[],
  ): Pick<DayTimeline, "prevDay" | "nextDay" | "firstDay"> {
      ...
      prevDay:
        idx > 0
          ...
          : sortedDates.length > 0
            ...
            : null,
      nextDay:
        idx >= 0 && idx < sortedDates.length - 1
          ? sortedDates[idx + 1]
          : sortedDates.length > 0
            ? sortedDates[0]
            : null,
  ...
  private getActiveDateCounts(
    sessions: Session[],
    sortedDates: string[],
  ): DayTimeline["activeDates"] {
...
function computeDaySpans(reqs: SessionRequest[]) {
  ...
  const avgStart =
    dayStartHours.length > 0
      ? dayStartHours.reduce((a, b) => a + b, 0) / dayStartHours.length
      : 9;
  const avgEnd =
    dayEndHours.length > 0
      ? dayEndHours.reduce((a, b) => a + b, 0) / dayEndHours.length
      : 17;
