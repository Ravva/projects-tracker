...
const PERSONAL_RULE_SOURCES: Record<
  string,
  { source: string; filePath: string }
> = {};
const PROJECT_RULE_SOURCES: Record<
  string,
  { source: string; filePath: string }
> = {};
 ...
 */
export function registerPersonalRuleSource(
  id: string,
  source: string,
  filePath: string,
): void {
 ...
 */
export function registerProjectRuleSource(
  id: string,
  source: string,
  filePath: string,
): void {
...
export function clearLayerRules(layer: "personal" | "project"): void {
  if (layer === "personal") {
    for (const k of Object.keys(PERSONAL_RULE_SOURCES))
      delete PERSONAL_RULE_SOURCES[k];
    personalRules = null;
  } else {
    for (const k of Object.keys(PROJECT_RULE_SOURCES))
      delete PROJECT_RULE_SOURCES[k];
    projectRules = null;
  }
}
...
function loadPersonalRules(): DetectionRule[] {
  ...
  personalRules = [];
  for (const [id, { source, filePath }] of Object.entries(
    PERSONAL_RULE_SOURCES,
  )) {
    const rule = parseRule(source);
    if (rule) {
 ...
 */
export function updateRuleThresholds(
  id: string,
...
export function getRulePreviewStats(
  ...
  return allRules
    ...
    .map((rule) => {
      ...
      const total =
        emission?.total ??
        (rule.scope === "sessions" ? sessions.length : reqs.length);
      ...
      return {
        ...
        group: rule.group,
        previewDescription:
          result?.description ?? "Not triggered with current data.",
