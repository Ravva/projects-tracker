...
export function isoWeek(d: Date): string {
  ...
  const wk = Math.ceil(
    ((thu.getTime() - new Date(yr, 0, 4).getTime()) / 86400000 +
      new Date(yr, 0, 4).getDay() +
      1) /
      7,
  );
...
const EFFORT_BEARING_RE =
  /^(?:claude-opus-4|claude-sonnet-4|gpt-5|o[1-9]|o\d{2,}|gemini-[2-9]|gemini-\d{2,})/;
...
const NON_EFFORT_SUFFIXES_RE =
  /-(fast|1m|mini|nano|micro|preview|latest|thought|turbo|flash|pro|ultra|internal)$/;
...
export function tokenCostInCredits(
  ...
  const cacheWriteCost =
    (cacheWriteTokens / 1_000_000) * (rates.cacheWrite ?? rates.input);
...
const WORK_PATTERNS: [RegExp, WorkType][] = [
  [
    /\b(fix|bug|error|issue|crash|exception|debug|problem|broken|fail|wrong)\b/i,
    "bug fix",
  ],
  [
    /\b(refactor|rename|extract|move|cleanup|simplify|restructure|reorganize)\b/i,
    "refactor",
  ],
  [
    /\b(review|pr|pull request|code review|comment on|feedback|approve)\b/i,
    "code review",
  ],
  [
    /\b(test|spec|expect|assert|mock|stub|coverage|vitest|jest|pytest|unittest)\b/i,
    "test",
  ],
  [
    /\b(doc|readme|comment|explain|jsdoc|typedoc|docstring|swagger|openapi)\b/i,
    "docs",
  ],
  [
    /\b(style|css|scss|sass|theme|layout|padding|margin|font|color|design|ui)\b/i,
    "style",
  ],
  [
    /\b(config|setup|install|dependency|package|ci|cd|pipeline|deploy|docker|k8s|terraform|bicep|env|yaml|yml)\b/i,
    "config",
  ],
  [
    /\b(add|create|implement|build|feature|new|scaffold|generate|develop)\b/i,
    "feature",
  ],
];
