...
export function parseAggregation(expr: string): MetricAggregation {
  ...
  const funcMatch = trimmed.match(
    /^(sum|avg|min|max|unique|percentile)\((\w[\w.]*)\s*(?:,\s*(\d+))?\)$/i,
  );
  if (funcMatch) {
  ...
  switch (agg.type) {
    ...
    case "sum":
      return matchedRows.reduce(
        (s, r) => s + toNum(resolveField(r, agg.field!)),
        0,
      );
    case "avg": {
      if (matchedRows.length === 0) return 0;
      const total = matchedRows.reduce(
        (s, r) => s + toNum(resolveField(r, agg.field!)),
        0,
      );
    ...
    case "min":
      return matchedRows.reduce(
        (m, r) => Math.min(m, toNum(resolveField(r, agg.field!))),
        Infinity,
      );
    case "max":
      return matchedRows.reduce(
        (m, r) => Math.max(m, toNum(resolveField(r, agg.field!))),
        -Infinity,
      );
    case "unique":
    ...
    case "percentile": {
      const values = matchedRows
        .map((r) => toNum(resolveField(r, agg.field!)))
        .sort((a, b) => a - b);
