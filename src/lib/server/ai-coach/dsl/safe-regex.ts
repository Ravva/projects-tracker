...
export function compileSafe(pattern: string, flags = ""): RegExp | null {
  ...
  if (typeof pattern !== "string" || pattern.length > MAX_PATTERN_LEN) {
    ...
    warnCore(
      "SafeRegex",
      `Pattern rejected (length > ${MAX_PATTERN_LEN}): ${pattern.slice(0, 40)}...`,
    );
    return null;
  }
  ...
  if (!isLikelySafe(pattern)) {
    ...
    warnCore(
      "SafeRegex",
      `Pattern rejected (potential catastrophic backtracking): ${pattern}`,
    );
    return null;
  }
  ...
  try {
    ...
    warnCore(
      "SafeRegex",
      `Invalid regex '${pattern}' flags='${flags}': ${(err as Error).message}`,
    );
    return null;
  }
...
export function testSafe(re: RegExp | null, input: string): boolean {
  if (!re) return false;
  const s =
    input.length > MAX_TEST_INPUT_LEN
      ? input.slice(0, MAX_TEST_INPUT_LEN)
      : input;
  try {
...
function hasUnboundedQuantifierAt(pattern: string, index: number): boolean {
  ...
  return (
    char === "+" ||
    char === "*" ||
    (char === "{" && /^\{\d+,\d*\}/.test(pattern.slice(index)))
  );
}
...
function closeGroup(
  ...
  if (group?.hasInnerUnbounded)
    return {
      safe: false,
      starHeight: nextStarHeight,
      maxStarHeight: nextMaxStarHeight,
    };
