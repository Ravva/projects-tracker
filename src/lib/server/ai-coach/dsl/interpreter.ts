...
import {
  containsProfanity as _containsProfanity,
  extractProfaneWords,
} from "../profanity";
...
function evaluateFieldAccess(
  node: ASTNode & { type: "field_access" },
...
function evaluateBinary(
  ...
  if (op === "AND")
    return toBool(evaluate(left, ctx)) && toBool(evaluate(right, ctx));
  if (op === "OR")
    return toBool(evaluate(left, ctx)) || toBool(evaluate(right, ctx));
...
function evaluateCall(
  ...
  const handler = (
    CALL_DISPATCH as Record<
      string,
      (argNodes: ASTNode[], ctx: Record<string, unknown>) => unknown
    >
  )[name];
...
function evaluatePipe(
  ...
  switch (filter) {
    ...
    case "ceil":
      return Math.ceil(toNum(value));
    case "abs":
      return Math.abs(toNum(value));
    case "pct":
      return `${(toNum(value) * 100).toFixed(numOpt(arg, 0))}%`;
    case "fixed":
      return toNum(value).toFixed(numOpt(arg, 2));
    case "locale":
...
const WORK_TYPE_PATTERNS: [RegExp, string][] = [
  ...
  [
    /\b(refactor|clean ?up|rename|restructure|reorganize|simplify)\b/i,
    "refactor",
  ],
  [/\b(test|spec|coverage|assert|expect|mock|stub)\b/i, "test"],
  [/\b(doc|readme|comment|jsdoc|typedoc|explain)\b/i, "documentation"],
  [
    /\b(deploy|ci|cd|pipeline|docker|kubernetes|helm|terraform|infra)\b/i,
    "devops",
  ],
...
function groupTop(
  ...
  return {
    key: topKey,
    count: topCount,
    share: arr.length > 0 ? topCount / arr.length : 0,
  };
}
...
function computeSpeedAcceptPairs(
  sessions: Record<string, unknown>[],
  minLoc: number,
  maxGap: number,
): { count: number; avgLoc: number; avgGap: number } {
  let count = 0;
  let totalLoc = 0;
  let totalGap = 0;
  for (const session of sessions) {
    ...
    for (let i = 0; i < reqs.length - 1; i++) {
      ...
      const aiLoc = aiCode.reduce(
        (s: number, b: unknown) =>
          s + toNum((b as Record<string, unknown>)?.loc),
        0,
      );
...
function computeFlowScoreStats(
  ...
  for (const session of sessions) {
    ...
    for (let i = 1; i < timed.length; i++) {
      const prevEnd =
        toNum(timed[i - 1].timestamp) + toNum(timed[i - 1].totalElapsed);
    ...
    // Group by day
    const day = new Date(toNum(timed[0].timestamp))
      .toISOString()
      .substring(0, 10);
  ...
  if (totalDays === 0)
    return { fragmentedDays: 0, totalDays: 0, avgScore: 100, lowScoreRate: 0 };
...
function computeLangExploration(reqs: Record<string, unknown>[]): {
  weeksSinceNew: number;
  totalLangs: number;
  recentNew: number;
  totalWeeks: number;
} {
  ...
  for (const r of reqs) {
    ...
    for (const field of ["aiCode", "userCode"] as const) {
      ...
      for (const b of blocks) {
        const lang = asStr(
          (b as Record<string, unknown>)?.language,
        ).toLowerCase();
  ...
  if (weeks.length === 0)
    return { weeksSinceNew: 0, totalLangs: 0, recentNew: 0, totalWeeks: 0 };
...
function computeMdRatio(
  ...
  for (const session of sessions) {
    const ws =
      asStr(session.workspaceName) || asStr(session.workspaceId) || "unknown";
  ...
  workspaces.sort((a, b) =>
    a.isLow === b.isLow ? b.codeLoc - a.codeLoc : a.isLow ? -1 : 1,
  );
...
const VSCODE_HARNESSES = new Set([
  "VS Code",
  "VS Code Insiders",
  "Local Agent",
...
function computeDevcontainerStats(
  ...
  terminalReqs: number;
  vscodeReqs: number;
  sandboxedTerminalReqs: number;
  totalTerminalReqs: number;
  terminalRate: number;
  vscodeSessionCount: number;
} {
  const vscodeSessions = sessions.filter((s) =>
    VSCODE_HARNESSES.has(asStr(s.harness)),
  );
  ...
  const vscodeReqs = reqs.filter((r) =>
    sessionIsContained.has(asStr(r.sessionId)),
  );
  ...
  for (const r of vscodeReqs) {
    ...
    if (
      !Array.isArray(confs) ||
      !confs.some((tc: unknown) => (tc as Record<string, unknown>).isTerminal)
    )
      continue;
    if (sessionIsContained.get(asStr(r.sessionId)) === true)
  ...
  return {
    ...
    totalTerminalReqs,
    terminalRate:
      vscodeReqs.length > 0 ? hostedTerminalReqs / vscodeReqs.length : 0,
...
function computeContextGaps(reqs: Record<string, unknown>[]): {
  gapCount: number;
  gaps: string[];
  reqCount: number;
} {
  ...
  const hasMcp = reqs.some((r) => {
    ...
    return (
      Array.isArray(tools) &&
      tools.some((t: unknown) => String(t).startsWith("mcp_"))
    );
  });
  ...
  if (total > 0 && fileRefCount / total < 0.1)
    gaps.push("Low file reference rate (< 10%)");
  ...
  if (total > 0 && instrCount / total < 0.05)
    gaps.push("Low custom instructions usage (< 5%)");
...
function computeYoloStats(reqs: Record<string, unknown>[]): {
  autoApproved: number;
  totalConfirmations: number;
  ratio: number;
  withConfirmationsCount: number;
} {
  let totalConf = 0;
  let autoApproved = 0;
  let withConf = 0;
...
function computeReasoningEffortStats(
  ...
  const premiumSet =
    premiumLevel === "max" ? new Set(["max"]) : new Set(["high", "max"]);
  let premiumCount = 0;
  let totalKnown = 0;
...
function computeExcessFileContextStats(
  ...
  const p95 =
    counts.length > 0
      ? counts[Math.min(counts.length - 1, Math.floor(counts.length * 0.95))]
      : 0;
  return {
...
function computeProfanityMatches(reqs: Record<string, unknown>[]): {
  count: number;
  total: number;
  flaggedWords: string[];
} {
...
function computeAutoApproveStats(reqs: Record<string, unknown>[]): {
  terminalAutoApproved: number;
  autoApprovedTotal: number;
  withConfirmations: number;
} {
  let withConf = 0,
    autoTotal = 0,
    termAuto = 0;
  ...
  return {
    terminalAutoApproved: termAuto,
    autoApprovedTotal: autoTotal,
    withConfirmations: withConf,
  };
}

/* ── Group by field and sum another, return top group ── */

function groupTopBySum(
  arr: unknown[],
  groupField: string,
  sumField: string,
): { key: string; sum: number; share: number; count: number } {
  const counts = new Map<string, number>();
  let totalSum = 0;
  for (const item of arr) {
  ...
  return {
    key: topKey,
    sum: topSum,
    share: totalSum > 0 ? topSum / totalSum : 0,
    count: counts.size,
  };
}
...
function computeModelStats(reqs: Record<string, unknown>[]): {
  topModel: string;
  topCount: number;
  topShare: number;
  modelCount: number;
...
> = {
  ...
  startsWith: (argNodes: ASTNode[], ctx: Record<string, unknown>) => {
...

    {
      const [s, sub] = args();
      return (
        typeof s === "string" && typeof sub === "string" && s.startsWith(sub)
      );
    }
  },
  endsWith: (argNodes: ASTNode[], ctx: Record<string, unknown>) => {
...

    {
      const [s, sub] = args();
      return (
        typeof s === "string" && typeof sub === "string" && s.endsWith(sub)
      );
    }
  },
  ...
  substr: (argNodes: ASTNode[], ctx: Record<string, unknown>) => {
...

    {
      ...
      return typeof s === "string"
        ? s.substring(
            toNum(start),
            len != null ? toNum(start) + toNum(len) : undefined,
          )
        : "";
    }
  },
  ...
  some: (argNodes: ASTNode[], ctx: Record<string, unknown>) => {
    {
      ...
      const fieldName =
        argNodes[1]?.type === "identifier"
          ? (argNodes[1] as { name: string }).name
          : asStr(evaluate(argNodes[1], ctx));
  ...
  has: (argNodes: ASTNode[], ctx: Record<string, unknown>) => {
...

    {
      ...
      return (
        ...
        obj &&
        !Array.isArray(obj) &&
        asStr(key) in (obj as Record<string, unknown>)
      );
    }
  ...
  iif: (argNodes: ASTNode[], ctx: Record<string, unknown>) => {
    {
      ...
      return cond
        ? evaluate(argNodes[1], ctx)
        : argNodes[2]
          ? evaluate(argNodes[2], ctx)
          : null;
    }
  },
  ...
  capsWordRatio: (argNodes: ASTNode[], ctx: Record<string, unknown>) => {
    {
      const text = asStr(evaluate(argNodes[0], ctx));
      const minLen =
        argNodes.length > 1 ? toNum(evaluate(argNodes[1], ctx)) : 3;
      let wordLen = 0,
        wordCount = 0,
        capsWordCount = 0;
      let allUpper = true,
  ...
  sumLoc: (argNodes: ASTNode[], ctx: Record<string, unknown>) => {
    {
      ...
      if (!Array.isArray(arr)) return 0;
      return arr.reduce<number>(
        (s, b) => s + toNum((b as Record<string, unknown>)?.loc),
        0,
      );
    }
  ...
  sumLocFor: (argNodes: ASTNode[], ctx: Record<string, unknown>) => {
...

    {
      ...
      const langSet = new Set(
        langs.map((l: unknown) => asStr(l).toLowerCase()),
      );
  ...
  substring: (argNodes: ASTNode[], ctx: Record<string, unknown>) => {
...

    {
      ...
      return typeof s === "string"
        ? s.substring(toNum(start), end != null ? toNum(end) : undefined)
        : "";
    }
  ...
  isStructured: (argNodes: ASTNode[], ctx: Record<string, unknown>) => {
    {
      ...
      return (
        ...
        /\b(requirements?|spec|acceptance criteria|user stor(y|ies)|given|when|then|should|must)\b/i.test(
          s,
        ) ||
  ...
  flatSumField: (argNodes: ASTNode[], ctx: Record<string, unknown>) => {
    {
      ...
      const arrayField =
        argNodes[1]?.type === "identifier"
          ...
          : asStr(evaluate(argNodes[1], ctx));
      const sumFieldName =
        argNodes[2]?.type === "identifier"
          ? (argNodes[2] as { name: string }).name
          : asStr(evaluate(argNodes[2], ctx));
      let total = 0;
      for (const item of arr) {
        ...
        if (Array.isArray(sub)) {
          for (const elem of sub) {
            total += toNum(
              resolveField(elem as Record<string, unknown>, sumFieldName),
            );
          }
        }
      }
  ...
  slice: (argNodes: ASTNode[], ctx: Record<string, unknown>) => {
    {
      ...
      const end =
        argNodes.length > 2 ? toNum(evaluate(argNodes[2], ctx)) : undefined;
  ...
  anyWhere: (argNodes: ASTNode[], ctx: Record<string, unknown>) => {
    {
      ...
      const op =
        argNodes.length >= 4 ? asStr(evaluate(argNodes[2], ctx)) : "==";
      const value =
        argNodes.length >= 4
          ? evaluate(argNodes[3], ctx)
          : evaluate(argNodes[2], ctx);
      return (arr as unknown[]).some((item) => {
        const row: Record<string, unknown> =
          typeof item === "object" && item
            ...
            : { ".": item };
        const fv: unknown =
          fieldName === "." ? item : resolveField(row, fieldName);
        switch (op) {
          case "==":
            return looseEquals(fv, value);
          case "!=":
            return !looseEquals(fv, value);
          case ">":
            return toNum(fv) > toNum(value);
          case ">=":
            return toNum(fv) >= toNum(value);
          case "<":
            return toNum(fv) < toNum(value);
  ...
  sumAiLoc: (argNodes: ASTNode[], ctx: Record<string, unknown>) => {
    {
      ...
      for (const r of reqs) {
        ...
        if (Array.isArray(blocks)) {
          for (const b of blocks)
            total += toNum((b as Record<string, unknown>)?.loc);
        }
      }
      return total;
    }
  ...
  hasReviewFollowup: (argNodes: ASTNode[], ctx: Record<string, unknown>) => {
    {
      ...
      const REVIEW_RE =
        /\b(change|fix|modify|update|refactor|wrong|instead|actually|revert|redo|try again)\b/i;
      for (let i = 1; i < reqs.length; i++) {
        const r = reqs[i] as Record<string, unknown>;
        const msg = asStr(r?.messageText);
        const edits = r?.editedFiles;
        if (REVIEW_RE.test(msg) || (Array.isArray(edits) && edits.length > 0))
          return true;
      }
      return false;
    }
  ...
  sumField: (argNodes: ASTNode[], ctx: Record<string, unknown>) => {
    {
      ...
      return arr.reduce<number>(
        (s, item) =>
          s + toNum(resolveField(item as Record<string, unknown>, fieldName)),
        0,
      );
    }
  },
  ...
  flatSomeWhere: (argNodes: ASTNode[], ctx: Record<string, unknown>) => {
    {
      ...
      const op =
        argNodes.length >= 5 ? asStr(evaluate(argNodes[4], ctx)) : "==";
      const checkElem = (elem: unknown): boolean => {
        const fv =
          subField === "."
            ? elem
            : resolveField(elem as Record<string, unknown>, subField);
        switch (op) {
          case "==":
            return looseEquals(fv, target);
          case "!=":
            return !looseEquals(fv, target);
          case "startsWith":
            return asStr(fv).startsWith(asStr(target));
          case "contains":
            return asStr(fv).includes(asStr(target));
          case "matches": {
  ...
  adjacentPairCount: (argNodes: ASTNode[], ctx: Record<string, unknown>) => {
    {
      ...
      return computeSpeedAcceptPairs(
  ...
  flowScoreStats: (argNodes: ASTNode[], ctx: Record<string, unknown>) => {
    {
      ...
      const minTimed =
        argNodes.length > 1 ? toNum(evaluate(argNodes[1], ctx)) : 3;
      const rapidMs =
        argNodes.length > 2 ? toNum(evaluate(argNodes[2], ctx)) : 30000;
      if (!Array.isArray(sessions))
        return { fragmentedDays: 0, totalDays: 0, avgScore: 100 };
      return computeFlowScoreStats(
        sessions as Record<string, unknown>[],
        minTimed,
        rapidMs,
      );
  ...
  langExplorationWeeks: (argNodes: ASTNode[], ctx: Record<string, unknown>) => {
    {
      ...
      if (!Array.isArray(reqs))
        return { weeksSinceNew: 0, totalLangs: 0, recentNew: 0 };
  ...
  mdRatioByWorkspace: (argNodes: ASTNode[], ctx: Record<string, unknown>) => {
    {
      const sessions = evaluate(argNodes[0], ctx);
      const minLoc =
        argNodes.length > 1 ? toNum(evaluate(argNodes[1], ctx)) : 100;
      const docLangs =
        argNodes.length > 2 ? evaluate(argNodes[2], ctx) : ["markdown", "md"];
      if (!Array.isArray(sessions))
        return { lowCount: 0, totalWorkspaces: 0, overallRatio: 0 };
  ...
  devcontainerStats: (argNodes: ASTNode[], ctx: Record<string, unknown>) => {
    {
      ...
      if (!Array.isArray(sessions) || !Array.isArray(reqs))
        return {
          terminalReqs: 0,
          vscodeReqs: 0,
          sandboxedTerminalReqs: 0,
          totalTerminalReqs: 0,
          terminalRate: 0,
        };
      return computeDevcontainerStats(
        sessions as Record<string, unknown>[],
  ...
  yoloStats: (argNodes: ASTNode[], ctx: Record<string, unknown>) => {
    {
      const reqs = evaluate(argNodes[0], ctx);
      if (!Array.isArray(reqs))
        return { autoApproved: 0, totalConfirmations: 0, ratio: 0 };
  ...
  reasoningEffortStats: (argNodes: ASTNode[], ctx: Record<string, unknown>) => {
    {
      ...
      const premiumLevel =
        argNodes.length > 1
          ? asStr(evaluate(argNodes[1], ctx), "high").toLowerCase()
          : "high";
      if (!Array.isArray(reqs))
        return {
          premiumCount: 0,
      ...
      return computeReasoningEffortStats(
        reqs as Record<string, unknown>[],
        premiumLevel,
      );
    }
  },
  instructionBloatStats: (
    argNodes: ASTNode[],
    ctx: Record<string, unknown>,
  ) => {
    {
      const sessions = evaluate(argNodes[0], ctx);
      const maxBytes =
        argNodes.length > 1 ? toNum(evaluate(argNodes[1], ctx)) : 4000;
      if (!Array.isArray(sessions))
        return {
          bloatedSessions: 0,
          withInstructionsCount: 0,
          totalSessions: 0,
          maxBytes: 0,
          p95Bytes: 0,
        };
      return computeInstructionBloatStats(
        sessions as Record<string, unknown>[],
        maxBytes,
      );
    }
  },
  excessFileContextStats: (
    argNodes: ASTNode[],
    ctx: Record<string, unknown>,
  ) => {
    {
      const reqs = evaluate(argNodes[0], ctx);
      const minFiles =
        argNodes.length > 1 ? toNum(evaluate(argNodes[1], ctx)) : 30;
      if (!Array.isArray(reqs))
        return {
          outlierCount: 0,
          totalReqs: 0,
      ...
      return computeExcessFileContextStats(
        reqs as Record<string, unknown>[],
        minFiles,
      );
    }
  ...
  duplicateGroups: (argNodes: ASTNode[], ctx: Record<string, unknown>) => {
    {
      const reqs = evaluate(argNodes[0], ctx);
      const minKeyLen =
        argNodes.length > 1 ? toNum(evaluate(argNodes[1], ctx)) : 10;
      const minCount =
        argNodes.length > 2 ? toNum(evaluate(argNodes[2], ctx)) : 3;
      if (!Array.isArray(reqs))
        return { totalDupes: 0, distinctCount: 0, topKey: "", topCount: 0 };
  ...
  hasConstraint: (argNodes: ASTNode[], ctx: Record<string, unknown>) => {
    {
      ...
      return /\b(do not|don't|dont|must not|mustn't|never|without|avoid|only|strictly|limit to|at most|at least|no more than|require|restrict|exclude|ensure|must|shall|should not|shouldn't)\b/i.test(
        sample,
      );
    }
  },
  ...
  isSpecDriven: (argNodes: ASTNode[], ctx: Record<string, unknown>) => {
    {
      ...
      if (
        Array.isArray(refs) &&
        refs.some((f) =>
          /\.(md|txt|spec|prd|design|plan|rfc|adoc)$/i.test(asStr(f)),
        )
      )
        return 1;
      ...
      if (
        /\b(spec|requirements?|acceptance criteria|design doc|PRD|RFC|plan file|constraint|must|should|ensure)\b/i.test(
          msg,
        )
      )
        return 1;
  ...
  hasPlanning: (argNodes: ASTNode[], ctx: Record<string, unknown>) => {
    {
      ...
      return (arr as Record<string, unknown>[]).some((r) => {
        ...
        return /\b(plan|architect|design|outline|approach|strategy|scope|breakdown|roadmap|RFC|spec|proposal)\b/i.test(
          msg,
        );
      })
        ? 1
