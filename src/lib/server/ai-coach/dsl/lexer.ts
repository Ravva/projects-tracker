...
function readTwoCharToken(input: string, index: number): ScanResult | null {
  ...
  return type === undefined
    ? null
    : createToken(type, input.slice(index, index + 2), index, index + 2);
}
...
function readNumberToken(
  ...
  if (!isDigit(firstChar) && !(allowLeadingMinus && firstChar === "-"))
    return null;
  let nextIndex = index + 1;
  while (
    nextIndex < input.length &&
    (isDigit(input[nextIndex]) || input[nextIndex] === ".")
  ) {
    nextIndex++;
  }
  return createToken(
    TokenType.NUMBER,
    input.slice(index, nextIndex),
    index,
    nextIndex,
  );
}

function readQuotedStringToken(
  input: string,
  index: number,
): ScanResult | null {
...
function readRegexToken(input: string, index: number): ScanResult {
  ...
  while (nextIndex < input.length && REGEX_FLAG_RE.test(input[nextIndex]))
    nextIndex++;
