...
function flushTurn(state: CLIParseState): void {
  ...
  const respTs = turn.lastAssistantTs
    ? new Date(turn.lastAssistantTs).getTime()
    : null;
  const firstProgress =
    turn.firstToolTs && msgTs ? turn.firstToolTs - msgTs : null;
  ...
  state.requests.push(
    createRequest({
      ...
      firstProgress:
        firstProgress !== null && firstProgress >= 0 ? firstProgress : null,
...
function handleSessionStart(
  ...
  state.currentReasoningEffort =
    canonicalizeReasoningEffort(str(data.reasoningEffort)) ??
...
function handleToolExecutionStart(ev: CLIEvent, state: CLIParseState): void {
  ...
  if (FILE_REF_TOOLS.has(toolName) && typeof args.path === "string")
    turn.referencedFiles.add(args.path);
  if (toolName === "skill" && typeof args.skill === "string")
    turn.skillsUsed.add(args.skill);
}

function handleToolExecutionComplete(
  data: Record<string, unknown>,
  state: CLIParseState,
): void {
  const turn = state.turn;
...
function addAssistantToolRequests(
  turn: TurnState,
...
function handleAssistantMessage(ev: CLIEvent, state: CLIParseState): void {
  ...
  if (typeof data.outputTokens === "number")
    turn.totalOutputTokens += data.outputTokens;
  addAssistantToolRequests(turn, data.toolRequests);
}

function handleCompactionComplete(
  data: Record<string, unknown>,
  state: CLIParseState,
): void {
  const turn = state.turn;
  ...
  turn.compaction = {
    ...
    numRoundsSinceLastSummarization:
      typeof data.numRoundsSinceLastSummarization === "number"
        ? data.numRoundsSinceLastSummarization
        : 0,
    contextLengthBefore:
      typeof data.contextLengthBefore === "number"
        ? data.contextLengthBefore
        : 0,
...
function addModelUsage(
  collected: Record<string, ModelUsage>,
  rawModel: string,
  info: unknown,
): void {
...
export function parseCLIEventsFile(
  ...
  const creationDate = state.startTime
