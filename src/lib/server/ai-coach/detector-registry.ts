...
function addWorkspaceDetails(
  ...
  if (!Array.isArray(extraWorkspaces) || extraWorkspaces.length === 0)
    return false;
...
function addSessionDetail(
  row: DetailRow,
  details: OccurrenceDetail[],
  weekCounts: Map<string, number>,
): void {
  const timestamp = numericValue(
    row.creationDate ?? row.lastMessageDate ?? row.startTime,
  );
...
function collectMatchedRowDetails(
  ...
  const addDetail =
    rule.scope === "sessions" ? addSessionDetail : addRequestDetail;
...
function emissionToAntiPattern(
  ...
  const hasWorkspaceDetails = addWorkspaceDetails(
    emission.extra?.workspaces,
    details,
  );
...
function buildWeeklyHist(weekCounts: Map<string, number>): {
  labels: string[];
  counts: number[];
} {
...
export const DETECTOR_REGISTRY: DetectorDefinition[] = new Proxy(
...
export function getActiveDetectors(
  ...
  return registry.filter(
    (detector) => !skipIdeDetectors || !detector.requiresIdeContext,
  );
}
 ...
 */
export function runEmitters(
  reqs: SessionRequest[],
  sessions: Session[],
  skipIdeDetectors: boolean,
): Map<string, DetectorEmission> {
  const rules = getAllRules();
  ...
  for (const rawRule of rules) {
    ...
    results.set(
      rule.id,
      executePipeline(pipeline, rule, { reqs, sessions, skipIdeDetectors }),
    );
  }
  return results;
}
