...
if (!port && !canUseProcessChannel)
  throw new Error("parse-worker: no parent channel");
...
onMessage(async (msg) => {
  try {
    ...
    const { result, dirMetas } = await parseAllLogsAsyncDetailed(
      logsDirs,
      (progress) => {
    ...
    runtimeDebug(
      "parse-worker",
      "message-result",
      `workspaces=${result.workspaces.size} sessions=${result.sessions.length}`,
    );
