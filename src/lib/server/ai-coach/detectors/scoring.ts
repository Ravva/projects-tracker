...
function codeReviewPenalty(request: SessionRequest): number {
  return request.aiCode.length > 0 &&
    request.totalElapsed != null &&
    request.totalElapsed < 5000
    ? 0.5
    : 0;
}

function toolMasteryPenalty(request: SessionRequest): number {
  return request.toolsUsed.length === 0 && request.slashCommand === ""
    ? 0.3
    : 0;
}
...
function weekScore(
  weekRequests: SessionRequest[],
  group: PracticeGroup,
): number {
  const total = weekRequests.length;
  if (total === 0) return 100;
  const badCount = weekRequests.reduce(
    (sum, request) => sum + requestPenalty(request, group),
    0,
  );
...
export function computeWeeklyTrend(reqs: SessionRequest[]): {
  labels: string[];
  counts: number[];
} {
...
export function computeWeeklyScores(reqs: SessionRequest[]): {
  labels: string[];
  series: { group: PracticeGroup; scores: number[] }[];
} {
