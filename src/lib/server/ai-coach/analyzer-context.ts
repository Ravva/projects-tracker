...
export class ContextAnalyzer extends AnalyzerBase {
  ...
  private computeVerdictThresholds(
    sessions: Session[],
  ): ContextVerdictThresholds {
    ...
    for (const s of sessions) {
      ...
      const avgUtil =
        (sessionTokens.reduce((a, b) => a + b, 0) /
    ...
    return {
      ...
      adaptive:
        optimal !== CONTEXT_OPTIMAL_UTILIZATION ||
        limited !== CONTEXT_LIMITED_UTILIZATION,
      sampleSize: avgUtils.length,
    };
  }
  ...
  private computeVerdict(
    ...
    const compactionRate =
      requestCount > 0 ? compactionCount / requestCount : 0;
    const degradedPeak = Math.min(
      95,
      Math.max(
        thresholds.optimalUtilization + 12,
        CONTEXT_SATURATION_THRESHOLD + 10,
      ),
    );
    const limitedPeak = Math.min(
      100,
      Math.max(thresholds.limitedUtilization + 10, degradedPeak + 10),
    );
    const degradedSaturation = 20;
    const limitedSaturation = 80;
    const degradedCompactions = compactionCount > 0;
    const limitedCompactions =
      compactionRate >= 0.12 ||
      (requestCount <= 20 && compactionCount >= CONTEXT_COMPACTION_STORM_MIN);

    if (
      avgUtil > thresholds.limitedUtilization ||
      peakUtil >= limitedPeak ||
      (saturation >= limitedSaturation &&
        avgUtil >= thresholds.limitedUtilization - 5) ||
      limitedCompactions
    ) {
      return "limited";
    }
    if (
      avgUtil > thresholds.optimalUtilization ||
      peakUtil >= degradedPeak ||
      saturation >= degradedSaturation ||
      degradedCompactions
    ) {
      return "degraded";
    }
  ...
  private estimateSessionContextWindow(session: Session): number {
    ...
    for (const r of session.requests) {
      if (r.promptTokens != null && r.promptTokens > maxNative)
        maxNative = r.promptTokens;
    }
    return maxNative > 0
  ...
  private estimateSessionTokens(session: Session): number[] {
    ...
    for (const r of session.requests) {
      out.push(
        r.promptTokens != null && r.promptTokens > 0 ? r.promptTokens : 0,
      );
    }
    return out;
  }
  ...
  getContextManagement(f?: DateFilter): ContextManagementData {
    ...
    const workspaceAnalyses = Array.from(
      this.groupSessionsByWorkspace(sessions).values(),
    )
      .map((group) =>
        this.analyzeWorkspace(group.ws, group.sessions, thresholds),
      )
      .filter(
    ...
    const totalCompactions = workspaceAnalyses.reduce(
      (sum, analysis) => sum + analysis.compactionCount,
      0,
    );
    const fullCompactions = workspaceAnalyses.reduce(
      (sum, analysis) => sum + analysis.fullCompactions,
      0,
    );
    const simpleCompactions = workspaceAnalyses.reduce(
      (sum, analysis) => sum + analysis.simpleCompactions,
      0,
    );
    const sessionsWithTokenData = workspaceAnalyses.reduce(
      (sum, analysis) => sum + analysis.sessionCount,
      0,
    );
    const overallScore = this.computeOverallWorkspaceScore(workspaces);
    const trend = this.buildWeeklyTrend(sessions, thresholds);
    const workspaceTrend = this.buildWorkspaceTrend(
      sessions,
      trend.map((t) => t.label),
      workspaces,
    );
    ...
    return {
      overallScore,
      estimatedContextWindow: this.estimateContextWindowFromRequests(
        this.filter(f),
      ),
      thresholds,
      workspaces,
  ...
  private estimateContextWindowFromRequests(
    requests: Array<{ promptTokens: number | null }>,
  ): number {
    let maxObserved = 0;
    for (const request of requests) {
      if (request.promptTokens != null && request.promptTokens > maxObserved)
        maxObserved = request.promptTokens;
    }
    return maxObserved > 0
      ? Math.max(
          CONTEXT_WINDOW_DEFAULT,
          Math.ceil(maxObserved / 32_000) * 32_000,
        )
      : CONTEXT_WINDOW_DEFAULT;
  }

  private groupSessionsByWorkspace(
    sessions: Session[],
  ): Map<string, { ws: Session; sessions: Session[] }> {
  ...
  private analyzeWorkspace(
    ...
    const avg =
      effectiveTokens.reduce((a, b) => a + b, 0) / effectiveTokens.length;
    ...
    const saturatedCount = utilizationSamples.filter(
      (u) => u >= CONTEXT_SATURATION_THRESHOLD,
    ).length;
    const saturation =
      Math.round((saturatedCount / utilizationSamples.length) * 1000) / 10;
    ...
    return {
      workspace: {
        ...
        compactionCount,
        costEfficiency: null,
        score: this.computeContextScore(
          avgUtil,
          saturation,
          fullCompactions,
          simpleCompactions,
  ...
  private getSessionLevelAvgTokens(session: Session): number {
    ...
    return totalInput > 0
      ? Math.round(totalInput / session.requests.length)
      : 0;
  }
  ...
  private computeContextScore(
    ...
    if (avgUtil <= thresholds.optimalUtilization) {
      ...
      const span = Math.max(
        1,
        thresholds.limitedUtilization - thresholds.optimalUtilization,
      );
    ...
    const compactionPenalty = Math.min(
      fullCompactions * 5 + simpleCompactions * 2,
      40,
    );
    return Math.max(
      0,
      Math.round(utilizationScore - saturationPenalty - compactionPenalty),
    );
  }

  private computeOverallWorkspaceScore(
    workspaces: WorkspaceContextScore[],
  ): number {
    const totalWeightedScore = workspaces.reduce(
      (sum, workspace) => sum + workspace.score * workspace.requestsWithTokens,
      0,
    );
    const totalWeight = workspaces.reduce(
      (sum, workspace) => sum + workspace.requestsWithTokens,
      0,
    );
  ...
  private buildWeeklyTrend(
    ...
    const weekMap = new Map<
      string,
      { utils: number[]; compactions: number; overThreshold: number }
    >();
      ...
      labels
        ...
        .map((label) => {
          ...
          const avgUtil =
            w.utils.length > 0
              ? Math.round(
                  (w.utils.reduce((a, b) => a + b, 0) / w.utils.length) * 10,
                ) / 10
              : 0;
          return {
            label,
  ...
  private detectContextAntiPatterns(
    sessions: Session[],
    thresholds: ContextVerdictThresholds,
  ): AntiPattern[] {
    ...
    if (bloatedSessions.length > 0) {
      const details: OccurrenceDetail[] = bloatedSessions
        .slice(0, 200)
        .map((b) => ({
          ...
          stats: {
            avgUtilPct: Math.round(b.avgUtil),
            requests: b.session.requestCount,
          },
        }));
      patterns.push({
        id: "context-bloat",
        ...
        suggestion:
          "Start new sessions more frequently. Break large tasks into smaller scoped sessions instead of continuing in one long thread.",
    ...
    if (compactionStormSessions.length > 0) {
      const details: OccurrenceDetail[] = compactionStormSessions
        .slice(0, 200)
        .map((c) => ({
          ...
          stats: {
            ...
            requests: c.session.requestCount,
          },
        }));
      patterns.push({
        id: "compaction-storm",
        ...
        suggestion:
          "Start fresh sessions before context becomes too large. Use @workspace or file references selectively rather than letting the context accumulate.",
        examples: compactionStormSessions
          .slice(0, 3)
          .map(
            (c) => `${c.session.workspaceName} (${c.compactions} compactions)`,
          ),
        details,
        weeklyHist: { labels: [], counts: [] },
      });
    ...
    for (const s of sessions) {
      for (const r of s.requests) {
        if (r.compaction && r.compaction.contextLengthBefore > 0) {
          const sessionCtx = this.estimateSessionContextWindow(s);
          const beforeUtil =
            (r.compaction.contextLengthBefore / sessionCtx) * 100;
          if (beforeUtil > 85) {
            amnesiaEvents.push({
              session: s,
              tokensBefore: r.compaction.contextLengthBefore,
            });
          }
        }
      }
    ...
    if (amnesiaEvents.length > 0) {
      const details: OccurrenceDetail[] = amnesiaEvents
        .slice(0, 200)
        .map((a) => ({
          ...
          kind: "session" as const,
          stats: {
            tokensK: Math.round(a.tokensBefore / 1000),
            requests: a.session.requestCount,
          },
        }));
      patterns.push({
        id: "context-amnesia",
        ...
        suggestion:
          "Summarize key decisions in a markdown file or use custom instructions so they survive compaction. Consider starting a new session with explicit context instead.",
        examples: amnesiaEvents
          .slice(0, 3)
          .map(
            (a) =>
              `${a.session.workspaceName}: ${Math.round(a.tokensBefore / 1000)}K tokens compacted`,
          ),
        details,
        weeklyHist: { labels: [], counts: [] },
      });
    ...
    for (const s of sessions) {
      ...
      if (growthRate > CONTEXT_GROWING_SESSION_GROWTH_RATE) {
        runawayEntries.push({
          session: s,
          first: sessionTokens[0],
          last: sessionTokens[sessionTokens.length - 1],
        });
      }
    }
    if (runawayEntries.length > 0) {
      const details: OccurrenceDetail[] = runawayEntries
        .slice(0, 200)
        .map((r) => ({
          ...
          kind: "session" as const,
          stats: {
            startK: Math.round(r.first / 1000),
            endK: Math.round(r.last / 1000),
            requests: r.session.requestCount,
          },
        }));
      patterns.push({
        id: "runaway-context",
        ...
        suggestion:
          "Reference only the files you need per request. Use /clear or start a new session periodically to reset context.",
        examples: runawayEntries
          .slice(0, 3)
          .map(
            (r) =>
              `${r.session.workspaceName}: ${Math.round(r.first / 1000)}K \u2192 ${Math.round(r.last / 1000)}K tokens`,
          ),
        details,
        weeklyHist: { labels: [], counts: [] },
      });
  ...
  getWorkspaceContextSessions(
    ...
    const sessions = this.filteredSessions(f).filter((s) =>
      this.matchesWorkspaceFilter(s, workspaceId),
    );
  ...
  private buildSessionContextDetail(
    ...
    if (!hasPerRequestTokens) {
      ...
      const compactionCount = session.requests.filter(
        (r) => r.compaction != null,
      ).length;
      ...
      return {
        ...
        requestQueries: session.requests.map((r) =>
          this.getRequestQueryPreview(r.messageText),
        ),
    ...
    const sessionCtx = this.estimateSessionContextWindow(session);
    const avg =
      tokensWithData.reduce((a, b) => a + b, 0) / tokensWithData.length;
    ...
    const saturatedCount = tokensWithData.filter(
      (t) => (t / sessionCtx) * 100 >= CONTEXT_SATURATION_THRESHOLD,
    ).length;
    const saturation =
      Math.round((saturatedCount / tokensWithData.length) * 1000) / 10;
  ...
  private getSessionDisplayDate(session: Session): string {
    ...
    if (session.lastMessageDate != null)
      return toDateStr(session.lastMessageDate);
    return "Unknown";
  }
    ...
    for (let i = 0; i < session.requests.length; i++) {
      ...
      if (request.compaction != null) {
        events.push({
          requestIndex: i,
          type: "compaction",
          label: `Compaction (${request.compaction.mode})`,
        });
      }
      if (request.todoSnapshot && request.todoSnapshot.length > 0) {
        prevSnapshot = this.addTodoSnapshotEvents(
          events,
          request.todoSnapshot,
          prevSnapshot,
          i,
        );
        continue;
  ...
  private addTodoSnapshotEvents(
    ...
    const currentSnapshot = new Map(
      snapshot.map((item) => [item.id, item.status]),
    );
    if (prevSnapshot == null) {
  ...
  private addToolFallbackTodoEvents(
    events: SessionTimelineEvent[],
    toolsUsed: string[],
    requestIndex: number,
  ): void {
    for (const tool of toolsUsed) {
      ...
      const isComplete =
        toolName.includes("complete") || toolName === "todoread";
      events.push({
        requestIndex,
  ...
  private generateTips(
    ...
    if (overallScore >= 80) {
      tips.push(
        "Excellent context management. Sessions stay within optimal utilization and avoid auto-compaction.",
      );
    } else if (overallScore >= 60) {
      tips.push(
        `Good context hygiene. A few workspaces could start fresh sessions earlier to stay under ${thresholds.optimalUtilization}% utilization.`,
      );
      ...
      tips.push(
        `Context is frequently at its limit. Start new sessions proactively and keep context under ${thresholds.optimalUtilization}% utilization.`,
      );
    }
    ...
    if (limited.length > 0) {
      tips.push(
        `${limited.length} workspace(s) have limited context capacity. Start fresh sessions before auto-compaction kicks in: ${limited
    ...
    if (highSaturation.length > 0) {
      tips.push(
        `${highSaturation.length} workspace(s) spend >30% of requests near the context ceiling. Even if averages look acceptable, sustained high utilization degrades quality.`,
      );
    }

    if (totalCompactions > 10) {
      tips.push(
        `${totalCompactions} compaction events detected. Auto-compaction means sessions ran too long — and mid-task compaction loses important context. Manually compact (or start a fresh session) at natural breakpoints: after a major research phase, a completed feature, or before switching focus.`,
      );
    } else if (totalCompactions > 0) {
      tips.push(
        `${totalCompactions} compaction event(s) detected. Monitor whether key instructions survive compaction.`,
      );
    }
    ...
    if (optimal.length > 0 && optimal.length < workspaces.length) {
      tips.push(
        `${optimal.length} workspace(s) are in the optimal range — study what makes them efficient and apply those patterns elsewhere.`,
      );
    }
  ...
  getContextRangeAvailability(f?: DateFilter): {
    ...
    for (const s of this.sessions) {
      ...
      harnessHasTokens.set(
        s.harness,
        prev === true ? true : sessionHasReqTokens,
      );
    }
    ...
    if (timestamps.length === 0) {
      return {
        rangesWithTokens: [],
        matchingSessions,
        sessionsWithRequestTokens,
        harnessesWithoutRequestTokens,
      };
    }
