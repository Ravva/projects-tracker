...
export function readFileSafe(filePath: string): string | null {
  try {
    ...
    if (stat.size > MAX_FILE_SIZE) {
      warnCore(
        "parser",
        `Skipping oversized file (${(stat.size / 1024 / 1024).toFixed(1)} MB): ${filePath}`,
      );
      return null;
    }
  } catch (e) {
...
export function createRequest(
  overrides: Partial<SessionRequest> &
    Pick<SessionRequest, "messageText" | "responseText">,
): SessionRequest {
  ...
  const sanitizedTs =
    overrides.timestamp != null && overrides.timestamp > 0
      ? overrides.timestamp
      : null;
...
export function createSession(
  overrides: Partial<Session> &
    Pick<
      Session,
      "sessionId" | "workspaceId" | "workspaceName" | "harness" | "requests"
    >,
): Session {
  const reqs = overrides.requests;
  const timestamps = reqs
    .map((r) => r.timestamp)
  ...
  if (merged.creationDate != null && merged.creationDate <= 0)
    merged.creationDate = computed.creationDate;
  if (merged.lastMessageDate != null && merged.lastMessageDate <= 0)
    merged.lastMessageDate = computed.lastMessageDate;
  return merged;
}

const DEVCONTAINER_PATH_RE = /(?:^|[\s=:"'`])\/workspaces\//;

export function detectDevcontainerFromRequests(
  ...
  for (const r of requests) {
    if (Array.isArray(r.toolConfirmations)) {
      for (const tc of r.toolConfirmations) {
        if (
          tc.isTerminal &&
          typeof tc.commandLine === "string" &&
          DEVCONTAINER_PATH_RE.test(tc.commandLine)
        ) {
          return true;
        }
      }
...
export function validateSession(
  ...
  warnCore(
    "schema",
    `Invalid session from ${source}: ${result.error.issues[0]?.message ?? "unknown"}`,
  );
  return null;
}
