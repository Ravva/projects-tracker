...
export async function compileNaturalLanguageRule(
  ...
  try {
    ...
    if (markdown) {
      ...
      notes.push(
        "LLM produced invalid rule markdown; falling back to heuristic.",
      );
    }
  } catch (err) {
    notes.push(
...
function buildSystemPrompt(): string {
  const fields = FIELD_SCHEMA.map(
    (f) => `  ${f.name}: ${f.type} — ${f.description}`,
  ).join("\n");
  const functions = FUNCTION_CATALOG.map(
...
function compileHeuristic(
  ...
  const { filterExpr, triggerExpr, thresholds } = guessExpressions(
    prompt,
    scope,
  );
...
function guessGroup(prompt: string): string {
  ...
  if (/session|long|mega|cancel|night|weekend|abandon/i.test(lower))
    return "session-hygiene";
  if (/code|review|accept|copy|paste|language|markdown/i.test(lower))
    return "code-review";
  if (/model|tool|skill|instruction|command|premium/i.test(lower))
    return "tool-mastery";
...
function guessExpressions(
  ...
  if (/night|late/i.test(lower)) {
    return {
      filterExpr:
        "hour(timestamp) >= {{thresholds.startHour}} OR hour(timestamp) < {{thresholds.endHour}}",
  ...
  if (/weekend/i.test(lower)) {
    return {
      ...
      triggerExpr:
        "ratio > {{thresholds.maxRate}} AND count > {{thresholds.minCount}}",
  ...
  if (/no\s*(file|context|reference)/i.test(lower)) {
    return {
      ...
      triggerExpr:
        "ratio > {{thresholds.maxRate}} AND count > {{thresholds.minSample}}",
