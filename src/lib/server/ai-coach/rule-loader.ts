...
export function registerAllBuiltinRules(): void {
  ...
  for (const [id, source] of Object.entries(BUILTIN_RULES)) {
 ...
 */
export function loadAllRuleLayers(
  workspaceRoot?: string,
  trustGate?: TrustGate,
): { builtin: number; personal: number; project: number } {
...
export function registerAllBuiltinMetrics(): number {
  ...
  let count = 0;
  for (const [id, source] of Object.entries(BUILTIN_METRICS)) {
    const metric = parseMetric(source);
    if (metric) {
      metric.source = "built-in";
      metric.sourceFilePath = `built-in-metrics://${id}.metric.md`;
 ...
 */
export function loadProjectMetrics(
  workspaceRoot: string,
  trustGate?: TrustGate,
): number {
 ...
 */
export function loadAllMetricLayers(
  workspaceRoot?: string,
  trustGate?: TrustGate,
): { builtin: number; personal: number; project: number } {
  clearMetrics();
  ...
  const project = workspaceRoot
    ? loadProjectMetrics(workspaceRoot, trustGate)
    : 0;
...
export async function loadPersonalRulesAsync(
  trustGate?: TrustGate,
): Promise<number> {
  clearLayerRules("personal");
  return loadRulesFromDirAsync(
    getPersonalRulesDir(),
    (id, source, filePath) => registerPersonalRuleSource(id, source, filePath),
    resolveGate(trustGate),
    "personal",
...
export async function loadProjectRulesAsync(
  workspaceRoot: string,
  trustGate?: TrustGate,
): Promise<number> {
  clearLayerRules("project");
  return loadRulesFromDirAsync(
    getProjectRulesDir(workspaceRoot),
    (id, source, filePath) => registerProjectRuleSource(id, source, filePath),
    resolveGate(trustGate),
    "project",
  );
...
export async function loadAllRuleLayersAsync(
  workspaceRoot?: string,
  trustGate?: TrustGate,
): Promise<{ builtin: number; personal: number; project: number }> {
  await registerAllBuiltinRulesAsync();
  const [personal, project] = await Promise.all([
    loadPersonalRulesAsync(trustGate),
    workspaceRoot
      ? loadProjectRulesAsync(workspaceRoot, trustGate)
      : Promise.resolve(0),
  ]);
  return {
    builtin: loadBuiltinRules().length,
...
export async function registerAllBuiltinMetricsAsync(): Promise<number> {
  return registerAllBuiltinMetrics();
}

export async function loadPersonalMetricsAsync(
  trustGate?: TrustGate,
): Promise<number> {
  return loadMetricsFromDirAsync(
    path.join(os.homedir(), ".ai-engineer-coach", "metrics"),
    "personal",
    resolveGate(trustGate),
    "personal",
  );
...
export async function loadProjectMetricsAsync(
  workspaceRoot: string,
  trustGate?: TrustGate,
): Promise<number> {
  return loadMetricsFromDirAsync(
    path.join(workspaceRoot, ".ai-engineer-coach", "metrics"),
    "project",
    resolveGate(trustGate),
    "project",
  );
...
export async function loadAllMetricLayersAsync(
  workspaceRoot?: string,
  trustGate?: TrustGate,
): Promise<{ builtin: number; personal: number; project: number }> {
  clearMetrics();
  ...
  const [personal, project] = await Promise.all([
    ...
    workspaceRoot
      ? loadProjectMetricsAsync(workspaceRoot, trustGate)
      : Promise.resolve(0),
  ]);
  return { builtin, personal, project };
}
