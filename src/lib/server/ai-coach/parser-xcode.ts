...
function collectDbFilesFromDir(convDir: string, dbFiles: string[]): void {
  try {
    ...
    console.debug(
      `No conversations dir in ${path.dirname(convDir)}:`,
...
function findXcodeDbFiles(xcodeBase: string): string[] {
  ...
  try {
    const machineIds = fs
      .readdirSync(xcodeBase, { withFileTypes: true })
    ...
    console.debug(
      `Cannot read Xcode dir ${xcodeBase}:`,
      e instanceof Error ? e.message : e,
    );
  }
  return dbFiles;
...
function sqliteQuery(dbPath: string, sql: string): string {
  ...
  try {
    ...
    console.debug(
      `sqlite3 query failed on ${dbPath}:`,
      e instanceof Error ? e.message : e,
    );
    return "";
  }
...
function sqliteQueryTurnsAsync(
  ...
  { rowID: number; id: string; role: string; data: string; createdAt: number }[]
> {
...
function parseAssistantReply(
  turns: XcodeTurnRow[],
  index: number,
): { assistantContent: string; assistantData: XcodeAssistantData | null } {
...
function extractToolsFromRounds(
  rounds: Array<{ toolCalls?: Array<{ name?: string }> }>,
): string[] {
...
export function parseXcodeDatabases(xcodeBase: string): Session[] {
  ...
  try {
    ...
    console.debug(
      "sqlite3 not available, skipping Xcode parsing:",
      e instanceof Error ? e.message : e,
    );
    return sessions;
  }
  ...
  for (const dbPath of dbFiles) {
    ...
    try {
      ...
      console.debug(
        `Failed to parse conversations from ${dbPath}:`,
        e instanceof Error ? e.message : e,
      );
      continue;
    }
    ...
    for (const conv of conversations) {
      ...
      try {
        ...
        console.debug(
          `Failed to parse turns for conversation ${conv.id}:`,
          e instanceof Error ? e.message : e,
        );
        continue;
      }
      ...
      const session = buildXcodeSession(conv, turns, (turnId, error) => {
        console.debug(
          `Failed to parse turn data for turn ${turnId}:`,
...
export async function parseXcodeDatabasesAsync(
  ...
  for (const dbPath of dbFiles) {
    let conversations: XcodeConversationRow[];
    try {
      const raw = await sqliteQueryAsync(
        dbPath,
        "SELECT id, title, createdAt, updatedAt FROM Conversation",
      );
      if (!raw.trim()) continue;
