...
function parseNestedValue(
  line: string,
  nestedObj: Record<string, number | string | string[]>,
): void {
...
function parseSingleCondition(expr: string): RuleCondition {
  const match = expr.match(
    /^(\S+)\s*(<=|>=|!=|<|>|==|=|includes|not-includes|matches|not-matches)\s*(.+)$/,
  );
...
function extractRuleMaps(fm: ParsedFrontmatter): {
  ...
  const rawThresholds = fm.thresholds as
  ...
  const rawPatterns = fm.patterns as
  ...
  const rawFileTypes = fm.fileTypes as
    | Record<string, string | string[]>
    | undefined;
  if (rawFileTypes) {
...
export function parseRule(markdown: string): DetectionRule | null {
  ...
  return {
    ...
    descriptionTemplate:
      descriptionTemplate || "{{count}} occurrences detected.",
 ...
 */
function parseTestFixtures(
  body: string,
): Array<{ input: Record<string, unknown>; expect: "triggered" | "clean" }> {
  const block = body.match(/```test\s*\n([\s\S]*?)```/);
  if (!block) return [];
  const tests: Array<{
    input: Record<string, unknown>;
    expect: "triggered" | "clean";
  }> = [];
...
function serializeCondition(c: RuleCondition): string {
  ...
  if (c.type === "ratio-check")
    return `check: ratio ${opToStr(c.op)} ${c.threshold}`;
  if (c.type === "count-check")
    return `check: count ${opToStr(c.op)} ${c.value}`;
  if (c.type === "aggregate")
    return `aggregate: ${c.aggregation || "count"}(${c.field || ""}) ${opToStr(c.op)} ${c.value}`;
