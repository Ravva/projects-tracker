...
export function evaluateMetric(
  ...
  if (thresholds) {
    resolvedFilter = fillTemplate(filterExpr, {
      thresholds,
  ...
  const count =
    agg.type === "count" || agg.type === "ratio" ? matched.length : value;
...
export function evaluateTrigger(
  ...
  if (thresholds) {
    resolved = fillTemplate(triggerExpr, { thresholds } as unknown as Record<
      string,
      unknown
    >);
  }
...
export function calibrate(
  ...
  return {
    ...
    flaggedPct:
      rows.length > 0
        ? Math.round((emission.count / rows.length) * 1000) / 10
        : 0,
    distribution,
    triggered,
...
export function serializeCalibration(cal: CalibrationResult): string {
  ...
  lines.push(
    `  flagged: ${cal.flaggedCount.toLocaleString()} (${cal.flaggedPct}%)`,
  );
  if (cal.distribution) {
    lines.push(
      `  p25: ${cal.distribution.p25}, p50: ${cal.distribution.p50}, p75: ${cal.distribution.p75}`,
    );
...
function extractSection(body: string, heading: string): string | null {
  ...
  const section =
    nextHeading >= 0
      ? body.substring(start, nextHeading)
