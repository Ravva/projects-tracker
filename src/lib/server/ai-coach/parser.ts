...
function computeTotalLoc(sessions: import("./types").Session[]): number {
  ...
  for (const s of sessions)
    for (const r of s.requests) for (const b of r.aiCode) total += b.loc;
  return total;
}
function computeTotalToolCalls(sessions: import("./types").Session[]): number {
  let total = 0;
  for (const s of sessions)
    for (const r of s.requests) total += r.toolsUsed.length;
  return total;
}
...
function computeTotalFilesEdited(
  ...
  for (const s of sessions)
    for (const r of s.requests) for (const f of r.editedFiles) seen.add(f);
  return seen.size;
}
...
function pct(phase: number, intraPhase: number): number {
  ...
  return Math.min(
    100,
    Math.round(base + width * Math.max(0, Math.min(1, intraPhase))),
  );
}
...
function partitionDirs(logsDirs: string[]): {
  vsCodeDirs: string[];
  xcodeDirs: string[];
} {
...
async function prefetchBatch(
  ...
  await Promise.allSettled(
    workItems.map(async ({ logsDir, wsId }) => {
      ...
      try {
        const editDirs = await fs.promises.readdir(
          path.join(wsPath, "chatEditingSessions"),
        );
        for (const d of editDirs) {
          if (filePaths.length >= MAX_PREFETCH_FILES) break;
          filePaths.push(
            path.join(wsPath, "chatEditingSessions", d, "state.json"),
          );
        }
      } catch {
        /* no editDir */
...
interface WorkerParseResponse {
  result: {
    ...
    sessionSourceIndex: [
      string,
      ParseResult["sessionSourceIndex"] extends Map<string, infer V>
        ? V
        : never,
    ][];
  };
...
function makeWorkspaceProgressKey(
  workspaceKey: string,
  wsId: string,
  order: number,
  date?: string,
...
async function collectWorkspaceSessionTiles(
    ...
    const stats = await Promise.allSettled(
      files.map(async (entry) => {
        ...
        return {
          mtime: stat.mtimeMs,
          size: stat.size,
          date: stat.mtimeMs > 0 ? toDateStr(stat.mtimeMs) : undefined,
        };
      }),
    );
  ...
  if (tiles.length === 0) {
    tiles.push({
      mtime: fallbackMtime,
      size: 0,
      date: fallbackMtime > 0 ? toDateStr(fallbackMtime) : undefined,
    });
  }
...
async function reportWorkspaceProgress(
  ...
  const shouldYield =
    elapsed > 2000 || processed % 4 === 0 || processed === totalDirs;
...
async function tryMemoryCache(
  ...
  mem.result.sessions = mem.result.sessions.filter(
    (s) => !EXTERNAL_HARNESS_SET.has(s.harness),
  );
  await collectExternalHarnesses(
    mem.result.workspaces,
    mem.result.sessions,
    onProgress,
  );
  report({
    phase: 1,
    detail: "Loaded from memory",
    pct: pct(1, 1),
...
async function tryDiskCache(
  ...
  cached.result.sessions = cached.result.sessions.filter(
    (s) => !EXTERNAL_HARNESS_SET.has(s.harness),
  );
  await collectExternalHarnesses(
    cached.result.workspaces,
    cached.result.sessions,
    onProgress,
  );
...
async function processWorkspaces(
  ...
  for (const { logsDir, dirEntries } of entries) {
    ...
    for (const d of dirEntries)
      work.push({
        ...
        harness,
        mtime: 0,
        workspaceKey: makeWorkspaceGroupKey(harness, d.name),
        sessionTiles: [],
      });
  }
  ...
  await Promise.allSettled(
    work.map(async (item) => {
      item.sessionTiles = await collectWorkspaceSessionTiles(
        item.logsDir,
        item.wsId,
        item.mtime,
      );
  ...
  try {
    for (let i = 0; i < work.length; i += BATCH_SIZE) {
      ...
      const nextPrefetch =
        nextBatch.length > 0 ? prefetchBatch(nextBatch) : Promise.resolve();

      let lastWsName = "";
      for (const { logsDir, wsId, harness, workspaceKey } of batch) {
        const start = Date.now();
        try {
          lastWsName = await processWorkspaceEntryAsync(
            logsDir,
            wsId,
            harness,
            ctx,
            (progress) => {
              ...
              onProgress({
                ...
                pct: pct(
                  2,
                  (processed + progress.completed / progress.total) / totalDirs,
                ),
                sessions: ctx.sessions.length,
...
export function parseAllLogs(logsDirs: string[]): ParseResult {
  ...
  for (const { logsDir, dirEntries } of entries) {
    const harness = harnessFromPath(logsDir);
    for (const d of dirEntries)
      processWorkspaceEntry(logsDir, d.name, harness, ctx);
  }
...
export async function parseAllLogsAsyncDetailed(
  ...
  const report: ReportProgress = (p) => {
    if (onProgress)
      onProgress({ detail: "", pct: pct(p.phase, 0), sessions: 0, ...p });
  };
  ...
  if (cached) {
    ...
    for (const fullPath of [...stale, ...removed])
      affectedWsIds.add(path.basename(fullPath));
    ...
    for (const s of cachedSessions) {
      if (
        affectedWsIds.has(s.workspaceId) ||
        EXTERNAL_HARNESS_SET.has(s.harness)
      ) {
    ...
    const staleWork = stalePaths.map((wsPath) => {
      ...
      return {
        ...
        wsId,
        harness,
        workspaceKey: makeWorkspaceGroupKey(harness, wsId),
        mtime,
        date,
        sessionTiles: [] as Array<{
          mtime: number;
          size: number;
          date?: string;
        }>,
    ...
    for (const { logsDir, wsId, harness, workspaceKey } of staleWork) {
      await processWorkspaceEntryAsync(logsDir, wsId, harness, {
        workspaces,
      ...
      if (done % 20 === 0 || done === stalePaths.length) {
        report({
          phase: 2,
          detail: `${done}/${stalePaths.length}`,
          pct: pct(2, done / stalePaths.length),
          sessions: freshSessions.length,
          workspaceDone: workspaceKey,
        });
        await yieldToLoop();
...
export async function parseAllLogsViaWorker(
  ...
  const runChildAttempt = (
    ...
    runtimeDebug(
      "parser",
      "child-start",
      `attempt=${attempt} logsDirs=${logsDirs.length} worker=${workerPath} maxOldSpaceMb=${maxOldSpaceMb}`,
    );
    ...
    return new Promise((resolve, reject) => {
      ...
      const timer = setTimeout(() => {
        runtimeDebug(
          "parser",
          "child-timeout",
          `attempt=${attempt} timeoutMs=${TIMEOUT_MS}`,
        );
      ...
      child.on(
        ...
        (
          msg:
            ...
            | { type: "result"; payload: WorkerParseResponse }
          ...
          if (msg.type === "progress") {
            if (msg.progress.phase !== lastPhase) {
              ...
              runtimeDebug(
                ...
                `attempt=${attempt} phase=${msg.progress.phase} detail=${msg.progress.detail || ""}`,
              );
            }
          ...
          if (msg.type === "result") {
            runtimeDebug(
              "parser",
              "child-result",
              `attempt=${attempt} workspaces=${msg.payload.result.workspaces.length} sessions=${msg.payload.result.sessions.length}`,
            );
            finish(() => {
              const result: ParseResult = {
                ...
                editLocIndex: new Map(
                  msg.payload.result.editLocIndex.map(([k, v]) => [
                    k,
                    new Map(v),
                  ]),
                ),
                sessionSourceIndex: new Map(
                  msg.payload.result.sessionSourceIndex,
                ),
              };
      ...
      child.on("exit", (code, signal) => {
        runtimeDebug(
          "parser",
          "child-exit",
          `attempt=${attempt} code=${code} signal=${signal || ""}`.trim(),
        );
        if (!settled) {
          const reason = signal
  ...
  try {
    ...
    const retryable =
      /heap out of memory|memory limit|sigabrt|sigkill|exited with code/i.test(
        message.toLowerCase(),
      );
    ...
    runtimeDebug(
      "parser",
      "child-retry",
      `reason=${message} maxOldSpaceMb=${RETRY_WORKER_MAX_OLD_SPACE_MB}`,
    );
