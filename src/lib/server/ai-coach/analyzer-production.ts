  ...
  getCodeProduction(f?: DateFilter): CodeProductionData {
    ...
    for (const request of reqs) {
      ...
      for (const block of request.aiCode) {
        ...
        this.addWorkspaceProductionLoc(
          wsAi,
          dailyWsAi,
          workspaceName,
          day,
    ...
    const langArr = Array.from(langAi.keys())
      .sort((a, b) => (langAi.get(b) || 0) - (langAi.get(a) || 0))
      .slice(0, 15);
    ...
    if (
      f?.fromDate &&
      f.fromDate > "0001-01-01" &&
      (dayKeys.length === 0 || f.fromDate < dayKeys.sort()[0])
    ) {
      dayKeys.push(f.fromDate);
    }
    const dayArr = fillDayRange(dayKeys);

    const wsArr = Array.from(wsAi.keys())
      .sort((a, b) => (wsAi.get(b) || 0) - (wsAi.get(a) || 0))
      .slice(0, 15);
