...
export class ImageAnalyzer extends AnalyzerBase {
  getImageGallery(f?: DateFilter): ImageGalleryData {
    ...
    for (const r of reqs) {
      ...
      const session =
        this.requestSessionMap.get(r) ?? reqSessionMap.get(r.requestId);
      if (!session) continue;
    ...
    for (const m of moments) {
      workspaceCounts.set(
        m.workspace,
        (workspaceCounts.get(m.workspace) ?? 0) + m.imageCount,
      );
    ...
    const dateRange =
      dates.length > 0 ? `${dates[dates.length - 1]} \u2013 ${dates[0]}` : "";
    ...
    return {
      ...
      summary: {
        ...
        avgImagesPerMoment:
          moments.length > 0 ? +(totalImages / moments.length).toFixed(1) : 0,
        topWorkspace,
        topModel,
        dateRange,
        dailyImages,
  ...
  private buildStories(moments: ImageMoment[]): ImageStory[] {
    ...
    for (const [sessionId, sessionMoments] of bySession) {
      ...
      const sorted = [...sessionMoments].sort(
  ...
  private detectQualityFlags(moments: ImageMoment[]): ImageQualityFlag[] {
