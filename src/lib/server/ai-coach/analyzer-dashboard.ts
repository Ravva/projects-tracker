...
function fm(name: string, label: string, category: FieldCategory): FieldMeta {
...
export class DashboardAnalyzer extends AnalyzerBase {
  ...
  getWorkspaces(): {
    ...
    const recent = all
      ...
      .map((w) => ({
        ...w,
        recent: true,
        harnesses: [...(wsHarnesses.get(w.id) || [])],
      }));
    const rest = all
      .filter((w) => !recentIds.has(w.id))
    ...
    for (const r of reqs) {
      ...
      if (session) {
        wsCount.set(
          ...
          (wsCount.get(session.workspaceName) || 0) + 1,
        );
      }
    }
    const sorted = Array.from(wsCount.entries())
      .sort((a, b) => b[1] - a[1])
      .slice(0, 20);
  ...
  getHarnessComparison(f?: DateFilter): HarnessComparisonData {
    ...
    const harnesses = Array.from(byHarness.entries())
      .map(([harness, hSessions]) =>
        this.buildHarnessComparisonItem(harness, hSessions),
      )
  ...
  private buildHarnessComparisonItem(
    ...
    return {
      ...
      avgRequestsPerSession:
        sessions.length > 0
          ? Math.round((totalReqs / sessions.length) * 10) / 10
          : 0,
      totalAiLoc: reqs.reduce((sum, r) => sum + this.requestLoc(r), 0),
      avgResponseLength:
        totalReqs > 0
          ? Math.round(
              reqs.reduce((sum, r) => sum + r.responseLength, 0) / totalReqs,
            )
          : 0,
      topModels: this.getTopNamedCounts(
        reqs.flatMap((r) => (r.modelId ? [r.modelId] : [])),
      ),
      topTools: this.getTopNamedCounts(reqs.flatMap((r) => r.toolsUsed)),
      avgElapsed: this.getAverageElapsed(reqs),
      cancelRate:
        totalReqs > 0 ? reqs.filter((r) => r.isCanceled).length / totalReqs : 0,
      activeDays: days.size,
      firstSeen:
        timestamps.length > 0 ? toDateStr(Math.min(...timestamps)) : null,
      lastSeen:
        timestamps.length > 0 ? toDateStr(Math.max(...timestamps)) : null,
    };
  }

  private getAverageElapsed(reqs: SessionRequest[]): number | null {
    const elapsedVals = reqs.flatMap((r) =>
      r.totalElapsed != null ? [r.totalElapsed] : [],
    );
  ...
  getParserCoverage(): ParserCoverageData {
    ...
    const matrix: Record<
      string,
      Record<string, { populated: number; total: number }>
    > = {};
    ...
    for (const field of PARSER_FIELDS) {
      ...
      for (const h of harnessNames) {
        ...
        const populated = reqs.filter((r) =>
          isFieldPopulated(r, field.name),
        ).length;
  ...
  getParserPreview(focusField?: string): ParserPreviewData {
    const byHarness = new Map<
      string,
      {
        session: { sessionId: string; workspaceName: string };
    ...
    for (const s of this.sessions) {
      ...
      for (let i = 0; i < s.requests.length; i++) {
        arr.push({
          session: { sessionId: s.sessionId, workspaceName: s.workspaceName },
    ...
    for (const [harness, entries] of byHarness) {
      ...
      for (const entry of entries) {
        ...
        const recency = entry.request.timestamp
          ? entry.request.timestamp / 1e12
          : 0;
...
function computeCalendarActivity(reqs: SessionRequest[]): CalendarActivityData {
  ...
  for (const [date, data] of dayData) {
    ...
    const focusScore = Math.min(
      100,
      Math.round((activeBuckets.size / spanBuckets) * 100),
    );
