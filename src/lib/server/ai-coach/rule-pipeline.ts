...
function buildRuleContext(rule: DetectionRule): Record<string, unknown> {
  ...
  if (rule.patterns && Object.keys(rule.patterns).length > 0)
    ctx.patterns = rule.patterns;
  // fileTypes.xxx → fileTypes object
  if (rule.fileTypes && Object.keys(rule.fileTypes).length > 0)
    ctx.fileTypes = rule.fileTypes;
...
export function executePipeline(
  ...
  if (pipeline.severityExpr) {
    try {
      ...
      /* ignore severity expression errors */
    }
  }
...
export function resolveInheritance(
  ...
  if (visited.has(rule.id)) {
    warnCore(
      "Pipeline",
      `Circular inheritance detected: ${[...visited].join(" -> ")} -> ${rule.id}`,
    );
    return rule;
  }
  visited.add(rule.id);
  ...
  if (!parent) {
    warnCore(
      "Pipeline",
      `Inheritance: parent rule '${rule.extendsRule}' not found for '${rule.id}'`,
    );
    return rule;
  }
  ...
  return {
    ...
    descriptionTemplate:
      rule.descriptionTemplate || resolvedParent.descriptionTemplate,
    suggestionTemplate:
      rule.suggestionTemplate || resolvedParent.suggestionTemplate,
    ...
    conditions:
      rule.conditions.length > 0 ? rule.conditions : resolvedParent.conditions,
