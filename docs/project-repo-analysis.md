...
2. Секция должна быть оформлена как Markdown-таблица с колонками `ID | Deliverable | Status | Weight`.
3. Допустимые статусы только канонические: `completed`, `in_progress`, `pending`, `blocked`.
4. Сумма всех `Weight` должна быть ровно `100`.
5. В progress идет только сумма `Weight` строк со статусом `completed`.
6. Если секция отсутствует, не парсится или сумма весов не равна `100`, процент считается невалидным и остается `0`.
