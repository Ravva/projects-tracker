...
## Компактный канон Memory Bank

- структура `memory_bank` канонизирована в `AGENTS.md` (раздел `4. Минимальная структура Memory Bank`) и подтягивается на `/my-project` напрямую из репозитория `projects-tracker` через GitHub raw URL с локальным fallback;
- soft caps (рекомендуемый максимум, не hard truncate): `projectbrief.md` 4 000 символов / 30 строк, `productContext.md` 1 500 / 10 строк, `activeContext.md` 1 500 / 10 строк, `progress.md` 1 500 / 15 строк, `docs/README.md` 4 000 / 50 строк; суммарно ≤ 12 500 символов, что вписывается в лимит Appwrite `49 000` для `inputSnapshotJson` без аварийной обрезки;
- при нарушении soft caps `analyzeProjectRepository` поднимает сигнал `memoryBankSizeWarning` и публикует фактические размеры файлов в `memoryBankFileChars`, а `serializeProjectAiInputSnapshot` дополнительно обрезает поле по границе последней строки и проставляет флаг в `truncatedFields`;
- `systemPatterns.md` и `techContext.md` в ученическом memory bank **не читаются AI**; их можно вести, но они не должны разрастаться;
- в `memory_bank` запрещены подпапки `modules/*`, `ui_extension/*`, `other/*` и любые длинные логи/дампы кода.
...
- `memoryBankSizeWarning`:
  хотя бы одно поле memory bank превысило соответствующий soft cap;
- `completion_percent`:
...
- `memory_bank_size_warning`:
  хотя бы один файл memory bank превысил свой soft cap (projectBrief > 4 000, productContext/activeContext/progress > 1 500, docsReadme > 4 000); флаг не блокирует AI-анализ, но служит сигналом, что ученик должен сократить memory bank;
- `abandoned`:
